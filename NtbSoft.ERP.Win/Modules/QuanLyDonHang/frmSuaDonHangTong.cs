...
namespace NtbSoft.ERP.Win.Modules.QuanLyDonHang
{
    public partial class frmSuaDonHangTong : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void bandedGridView2_ShowingEditor(object sender, CancelEventArgs e)
        {
            ...
            bool isNewRow = false;
            object isNewValue = gridView.GetRowCellValue(gridView.FocusedRowHandle, "IsNew");
            isNewRow = isNewValue != null && isNewValue != DBNull.Value && Convert.ToBoolean(isNewValue);
            if (fieldName == "NgayDKVC" || fieldName == "NgayThucTeVC" || fieldName == "NgayXuatHang")
            {
                if (!isNewRow)
                {
                    e.Cancel = true;
                ...
                && int.TryParse(rowFocused["IsCD"].ToString(), out parse))
            {
                ...
                if (SLTH == 1 && gridView.FocusedColumn.FieldName != "NgayGH" && gridView.FocusedColumn.FieldName != "NgayDKVC" && gridView.FocusedColumn.FieldName != "NgayThucTeVC" && gridView.FocusedColumn.FieldName != "NgayXuatHang" && gridView.FocusedColumn.FieldName != "GhiChu"
            ...
            if (_status == ResourceURL.EventStatus.View)
            {
                ...
                if (e.Column.FieldName == "NgayGH" || e.Column.FieldName == "NgayDKVC" || e.Column.FieldName == "NgayThucTeVC" || e.Column.FieldName == "NgayXuatHang" || e.Column.FieldName == "MaQG" || e.Column.FieldName == "GhiChu" || e.Column.FieldName == "ColorCode")
                {
                    ...
                    if (dhTongPOChiTiet.Count > 0)
                    {
                        foreach (BandedGridColumn col in view.Columns)
                        {
                            ...
                            BangSizeEntity bangsizeMT = (from l in lstBangSize
                                                         where l.MaKH.ToString().Trim().ToUpper() == donHangTong.MaKH.ToString().Trim().ToUpper() && l.MaHang == donHangTong.MaHang.ToString() && l.SizeSanXuat == sizeMTID.ToString() && l.MaNhomSize == DauSizeID
                    ...
                    foreach (BandedGridColumn col in view.Columns)
                    {
                        ...
                        BangSizeEntity bangsizeMT = (from l in lstBangSize
                                                     where l.MaKH.ToString().Trim().ToUpper() == donHangTong.MaKH.ToString().Trim().ToUpper() && l.MaKH.ToString().Trim().ToUpper() == donHangTong.MaKH.ToString().Trim().ToUpper() && l.MaHang.ToString().Trim().ToUpper() == donHangTong.MaHang.ToString().Trim().ToUpper() && l.SizeSanXuat == sizeMTID.ToString() && l.MaNhomSize == DauSizeID
                                                     select l).FirstOrDefault();
                        if (bangsizeMT == null) continue;
        ...
        private void SetGridBandedViewAmount(BandedGridView bandedView, string GridBandHeader, string GridBandCaption, List<string> columnNames)
        {
            ...
            if (nrOfColumns == 1 && (columnNames[0] == "MaDH" || columnNames[0] == "POID" || columnNames[0] == "PO" || columnNames[0] == "ColorCode" || columnNames[0] == "MaMau" || columnNames[0] == "DauSizeID" || columnNames[0] == "DauSize" || columnNames[0] == "MaQG" || columnNames[0] == "NgayGH" || columnNames[0] == "NgayDKVC" || columnNames[0] == "NgayThucTeVC" || columnNames[0] == "NgayXuatHang" || columnNames[0] == "GhiChu"))
            {
                ...
                else if (columnNames[0] == "NgayDKVC")
                {
                    ...
                    bandedColumns.OptionsColumn.ReadOnly = true;
                ...
                else
                {
                    ...
                    if (columnNames[0] == "DauSizeID")
                    {
                        ...
                        string urlDSize = string.Format("{0}?maHang={1}&&makh={2}", URL + "DonHangTong/GetDauSize", donHangTong.MaHang, donHangTong.MaKH);
        ...
        private void BtSave()
        {
            ...
            string urlGC = string.Format("{0}?mahang={1}&&makh={2}&&madh={3}&&username={4}", URL + "DonHangTong/GhiLogSuaDH", searchLookUpEditMH.EditValue.ToString(), searchLookUpEditKH.EditValue.ToString(), txtMaDH.Text.ToString(), GlobleData.UserName);
            string jsonMau = Task.Run(async () => { return await _clientExtension.GetAsnyc(urlGC); }).Result;
            BtNapLai();
            DevExpress.XtraSplashScreen.SplashScreenManager.CloseForm(false);
        }
        ...
        private void focused(object sender)
        {
            ...
            if (fieldName == "Amount" || fieldName == "ColorCode" || fieldName == "NgayDKVC" || fieldName == "NgayThucTeVC" || fieldName == "NgayXuatHang")
            {
        ...
        private void SaveDonHangTongPOAdd(DataTable tbl)
        {
            ...
            if (tbl != null && tbl.Rows.Count > 0)
            {
                ...
                if (lstTongPO != null && lstTongPO.Count > 0)
                {
                    ...
                    Console.WriteLine(result);
                    string urlUpdateGhiChu = string.Format("{0}?madh={1}", URL + "CanDoiDonHangTong/UpdateGhiChuQLSX", txtMaDH.Text.ToString());
                ...
                try
                {
                    ...
                    if (foucsedDelete >= 0)
                    {
                        ...
                        if (row != null)
                        {
                            ...
                            string urlislsx = string.Format("{0}?madh={1}&&poid={2}", URL + "CanDoiDonHangTong/GetIsLSX", txtMaDH.Text.ToString(), _poID.ToString());
        ...
        {
            frmXoaSize frm = new frmXoaSize(txtMaDH.Text.ToString());
            frm.ShowDialog();
            BtNapLai();
        }
