...
{
    public partial class frmERPKeHoachDongThung_TiLe : DevExpress.XtraEditors.XtraForm
    {
        ...
        int SoTiLe = 0;
        public frmERPKeHoachDongThung_TiLe(string MaDH, string MaDHDisplay, string MaHang, string TenHang, string POID, string MaPKL)
        {
            InitializeComponent();
        ...
        private void ProcessPackage()
        {
            try
            {
                ...
                List<Tile_DongThungEntity> lstTiLe = new List<Tile_DongThungEntity>();
                ...
                int gl_PCB_Pack = Convert.ToInt16(txtPack_Ctn.Text);
                            ...
                            foreach (var itemData in _dtDistinctDT)
                            {
                                ...
                                foreach (DataRow dr in _dtTemp.Rows)
                                {
                                    ...
                                    var _dtTempPCB = _dtTemp11.AsEnumerable().Where(x => x["SizeTypeID"].ToString() == itemData.SizeTypeID.ToString() &&
                                                                                     x["SizeID"].ToString() == dr["SizeID"].ToString()).CopyToDataTable();
                                    var SLPCB = _dtTempPCB.Rows[0]["SLPCB"].ToString() == "" ? 0 : _dtTempPCB.Rows[0]["SLPCB"];
                                    PCB_Pack = Convert.ToInt32(_dtTempPCB.Rows[0]["PCB_Pack"]);
                                    Pack_Ctn = Convert.ToInt32(_dtTempPCB.Rows[0]["Pack_Ctn"]);
                                    ...
                                    lstTiLe.Add(new Tile_DongThungEntity()
                                    {
                                        SizeID = dr["SizeID"].ToString(),
                                        Size = dr["Size"].ToString(),
                                        SLDT = SLDT,
                                        SLThung = Math.Floor( SLPCB.ToString() == "0" ? 0 : Convert.ToDouble(SLDT) / (Convert.ToInt16(SLPCB) * gl_PCB_Pack)),
                                        PCB = Convert.ToInt16(SLPCB),
                                        NW = NW,
                                        SLConLai = SLDT,
                                        PCB_Pack = PCB_Pack,
                                        Pack_Ctn = gl_PCB_Pack
                                    });
                                }
                                p++;
                                lstTiLe = lstTiLe.OrderBy(x => x.SLThung).ToList();
                                ProcessTL(lstTiLe, _dr, _dtData, ref _tuthung, ref _dednthung, TLThungChan, ref _sttThung, PCB_Pack, gl_PCB_Pack);
                                continue;
                                #region Tạm khóa
                                ...
                                double TLThungChan, ref int SttThung, int PCB_Pack, int Pack_Ctn)
        {
            try
            {
                if (lstTiLe.Count == 0) return;
                var temp = lstTiLe.Where(x => x.SLThung != 0).ToList()[0];
                double totalpiece = 0;
                double sumKL = 0;
                int SumPCB_Pack = 0;
                double Pack_Ctn_R = Pack_Ctn;
                foreach (var item in lstTiLe)
                {
                    if (item.SLConLai == 0) continue;
                    var valueCheck = item.SLConLai / (item.PCB * Pack_Ctn);
                    if (valueCheck < 1) valueCheck = 1;
                    if (Pack_Ctn_R == 0 || Pack_Ctn_R > valueCheck) Pack_Ctn_R = valueCheck > Pack_Ctn ? Pack_Ctn : valueCheck;
                }
                foreach (var item in lstTiLe)
                {
                    ...
                    SumPCB_Pack += tempPCB;
                    totalpiece += tempPCB * temp.SLThung * Pack_Ctn_R;
                    sumKL += tempPCB * temp.SLThung * item.NW;
                    item.SLConLai = Convert.ToInt32(item.SLDT - tempPCB * temp.SLThung * Pack_Ctn_R);
                }
                TuThung = DenThung + 1;
                DenThung = Convert.ToInt32(DenThung + temp.SLThung);
                _dr["PCB_Pack"] = SumPCB_Pack;
                _dr["Pack_Ctn"] = Pack_Ctn_R;
                ...
                if (SumDu > 0)
                {
                    ...
                    foreach (var item in lstTiLe)
                    {
                        item.SLThung = (item.SLConLai <= 0 || item.PCB == 0) ? 0 : (item.SLConLai < item.PCB) ? 1 : Math.Floor((item.SLConLai / (item.PCB * Pack_Ctn_R)));
                        item.SLThung = (item.SLThung < 1 && item.SLConLai > 0) ? 1 : item.SLThung;
                ...
                foreach (DataRow dr in _dtData.Rows)
                {
                    ...
                    for (int i = 0; i < slThung; i++)
                    {
                        for (int z = 0; z < _dtData.Columns.Count; z++)
                        {
                            ...
                            drNewRow["PCB_Pack"] = dr["PCB_Pack"].ToString();
                            drNewRow["Pack_Ctn"] = dr["Pack_Ctn"].ToString();
        ...
        private void btnSaveBarcode_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            SaveCaiDatBarcode();
        }
        private void SaveCaiDatBarcode()
        {
            DataTable tblSave = KHDongThungLib.CreateTblCaiDatBarcode();
            this.ActiveControl = grcDetail;
            var dtSource = dgrKHDongThung.DataSource as DataTable;
            if (dtSource is null || dtSource.Rows.Count == 0) return;
            foreach (DataRow dr in dtSource.Rows)
            {
                if (dr["Barcode"].ToString() == "") continue;
                string sizeID = "";
                var drSave = tblSave.NewRow();
                drSave["ID"] = 0;
                drSave["StyleID"] = dr["MaHang"];
                drSave["Season"] = dr["Dot"];
                drSave["POID"] = dr["POID"];
                ...
                foreach (DataColumn dc in dtSource.Columns)
                {
                    ...
                    if (!dc.ColumnName.Contains('@')) continue;
                    if(dr[dc].ToString() == "0") continue;
                    sizeID += ", " + dc.ColumnName.Split('@')[1];
                }
                sizeID = sizeID.Substring(2);
                drSave["SizeID"] = sizeID;
                drSave["BarCodeChan"] = dr["Barcode"];
                drSave["BarCodeLe"] = "";
                drSave["SoLuongLe"] = 0;
        ...
        private DataTable CreateTblCaiDatBarcode()
        {
            DataTable dt = new DataTable();
        ...
        private void CalculateSlTong(bool flagChangPCB = false)
        {
            try
            {
                if (txtSLTong.Text == "" || txtSLTong.Text == "0") return;
                var _dt = grcDetail.DataSource as DataTable;
                var _dtSize = gridControlSize.DataSource as DataTable;
                var SlTongLap = Convert.ToInt32(txtSLTong.Text);
                var dtSumKH = _dt.AsEnumerable().Sum(x => Convert.ToInt16(x["SLKH"]));
                var dtSumDaLap = _dt.AsEnumerable().Sum(x => Convert.ToInt16(x["SLDaLapPKL"]));
                if (SlTongLap > dtSumKH - dtSumDaLap)
                {
                    MessageBox.Show("Số lượng nhập vượt số lượng cho phép!");
                    return;
                }

                var dtPCBTemp = _dt.AsEnumerable().Select(x => new { SizeID = x["SizeID"].ToString(), PCB = x["SLPCB"].ToString() }).Distinct();
                int sumPCB = 0;
                if (flagChangPCB)
                {
                    //this.ActiveControl = grcDetail;

                    sumPCB = _dtSize.AsEnumerable().Sum(x => Convert.ToInt16(x["SLPCB"]));
                }
                else sumPCB = dtPCBTemp.AsEnumerable().Sum(x => Convert.ToInt16(x.PCB));
                SoTiLe = SlTongLap / sumPCB;
                foreach (DataRow dr in _dt.Rows)
                {
                    if (flagChangPCB)
                    {
                        var drSizeCT = _dtSize.AsEnumerable().Where(x => x["SizeID"].ToString() == dr["SizeID"].ToString()).FirstOrDefault();
                        dr["SLDT"] = Convert.ToInt16(drSizeCT["SLPCB"]) * SoTiLe;
                        continue;
                    }
                    if ((Convert.ToInt16(dr["SLPCB"]) * SoTiLe) > (Convert.ToInt16(dr["SLKH"]) - Convert.ToInt16(dr["SLDaLapPKL"])))
                    {
                        continue;
                    }
                    dr["SLDT"] = Convert.ToInt16(dr["SLPCB"]) * SoTiLe;
                }
            }
            catch (Exception ex)
            {
        ...
        private void txtSLTong_Leave(object sender, EventArgs e)
        {
        ...
        private void CalculateSlTong_ChangePCB()
        {
            try
            {
                if (txtSLTong.Text == "" || txtSLTong.Text == "0") return;
                var _dt = gridControlSize.DataSource as DataTable;
                var SlTongLap = Convert.ToInt32(txtSLTong.Text);

                //var dtPCBTemp = _dt.AsEnumerable().Select(x => new { SizeID = x["SizeID"].ToString(), PCB = x["SLPCB"].ToString() }).Distinct();
                var sumPCB = _dt.AsEnumerable().Sum(x => Convert.ToInt16(x["SLPCB"]));
                SoTiLe = SlTongLap / sumPCB;
                foreach (DataRow dr in _dt.Rows)
                {
                    dr["SLDT"] = Convert.ToInt16(dr["SLPCB"]) * SoTiLe;
        ...
        private void gridView3_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            {
                CalculateSlTong_ChangePCB();
                CalculateSlTong(true);
                DataRow dr = grvSize.GetFocusedDataRow();
                dr["SLThung"] = Convert.ToDouble(dr["SLPCB"].ToString() == "" ? 0 : dr["SLPCB"]) == 0 ? 0 : Math.Ceiling(Convert.ToDouble(dr["SLDT"]) / (Convert.ToDouble(dr["SLPCB"]) * Convert.ToInt16(txtPack_Ctn.Text)));
            }
