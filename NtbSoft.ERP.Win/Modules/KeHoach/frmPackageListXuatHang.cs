...
{
    public partial class frmPackageListXuatHang : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmPackageListXuatHang(string maPKLXH = "")
        {
            InitializeComponent();
        ...
        private void CreateDefault()
        {
            ...
            searchLookUpEdit_MaPKL.Properties.DisplayMember = "MaPKL";
            searchLookUpEdit_MaPKL.Properties.NullText = "[Chọn PKL]";

            searchLookUpEdit_MaPKL_Ton.Properties.ValueMember = "MaPKL";
            searchLookUpEdit_MaPKL_Ton.Properties.DisplayMember = "MaPKL";
        ...
        private void LoadKHDongThung(string controller, string madh, string maPKL)
        {
            ...
            string url = string.Format("{0}", URL + $"{controller}/Get?Action=GetPivotKHDongThung&MaDH={madh}&MaDVSX=Para&DotSX=Para&POID={_poid}&SizeTypeID={1}&ColorID=Para&ProductID=Para&SizeID=Para&MaPKL={maPKL}");
            string json = Task.Run(async () => { return await _clientExtension.GetAsnyc(url); }).Result;
        ...
        private void GetDataToPKL()
        {
            ...
            foreach (DataRow dr in dtXuatHang.Rows)
            {
                ...
                var SttDaXuat = dr["Stt_size"].ToString() == "1" ? Convert.ToInt16(dr["SttDaXuat_Decat"]) : Convert.ToInt16(dr["SttDaXuat"]);
        ...
        private void SaveXuatHang()
        {
            try
            {
                ...
                var _dtDataTempNew = _dtTempData.AsEnumerable().Where(x => !(bool)x["IsTon"]);
                var _dtDataTempTon = _dtTempData.AsEnumerable().Where(x => (bool)x["IsTon"]);
                ...
                PackageListXuatHangEntity listSaveXH = new PackageListXuatHangEntity();
                ...
                TenHang = _dtData.AsEnumerable().Select(x => x["TenHang"].ToString()).Distinct().Aggregate((s1, s2) => s1 + "," + s2);
                ...
                foreach (var itemMaDH in distinctDH)
                {
                    ...
                    {
                        int carton = _dtData.AsEnumerable().Where(y => y["MaDH"].ToString() == itemMaDH.MaDH && y["POID"].ToString() == itemPO.POID).Sum(x => Convert.ToInt32(x["SLThung"]));
                        int SLPCS = _dtData.AsEnumerable().Where(y => y["MaDH"].ToString() == itemMaDH.MaDH && y["POID"].ToString() == itemPO.POID).Sum(x => Convert.ToInt32(x["TotalPiece"]));
                        var drNew = tblSaveXuatHang.NewRow();
                        drNew["Id"] = 0;
                        drNew["MaPKL_XH"] = _maPKLXH;
                        drNew["MaDH"] = itemMaDH.MaDH;
                        drNew["MaHang"] = itemMaDH.Tenhang;
                        drNew["KhachHang"] = itemMaDH.KhachHang;
                        drNew["PO"] = itemPO.PO;
                        drNew["Carton"] = carton;
                        drNew["SoLuong"] = SLPCS;
                        drNew["Seal"] = txtSeal.Text;
                        drNew["Cont"] = txtCont.Text;
                        drNew["PackDate"] = DateTime.Now;
                        drNew["FinishDate"] = DateTime.Now;
                        drNew["CreateDate"] = DateTime.Now;
                        drNew["ExportDate"] = DateTime.Now;
                        drNew["NhanVien"] = GlobleData.UserName ?? "Test";
                        tblSaveXuatHang.Rows.Add(drNew);
                    }
                }
               
                DataTable tblSave = CreateTblSave();
                DataTable tblSaveTonKho = CreateTblSave();
                ...
                foreach (DataRow dr in _dtDataTon.Rows)
                {
                    int slThung = Convert.ToInt32(dr["SLThung"].ToString());
                    int TuThung = Convert.ToInt32(dr["TuThung"].ToString());
                    int DenThung = Convert.ToInt32(dr["DenThung"].ToString());
                    ...
                    foreach (DataColumn dc in _dtData.Columns)
                    {
                        ...
                        for (int i = 0; i < slThung; i++)
                        {
                            DataRow drNewRow = tblSaveTonKho.NewRow();
                            drNewRow["ID"] = 0;
                            drNewRow["MaDH"] = dr["MaDH"].ToString();
                            drNewRow["MaPKL"] = dr["MaPKL"].ToString();
                            drNewRow["POID"] = dr["POID"].ToString();
                            ...
                            tblSaveTonKho.Rows.Add(drNewRow);
                        }
                    }
                }
        ...
        private void DeleteRow(DataRow dr)
        {
            try
            {
                ...
                string url = string.Format("{0}", URL + $"XuatHang/Delete?action=DeletePKLXH_Row&Para={_maPKLXH}");
                string result = Task.Run(async () => { return await _clientExtension.PostAsync(url, ds); }).Result;
            }
            catch (Exception ex)
            {
        ...
        {
            if (e.Column.FieldName == "SLTDaXuat" || e.Column.FieldName == "SLXuat") return;
        ...
        private void searchLookUpEdit_MaPKL_Ton_EditValueChanged(object sender, EventArgs e)
        {
            ...
            var maPKL = searchLookUpEdit_MaPKL_Ton.EditValue.ToString();
            var madh = dtPKLTon.AsEnumerable().Where(x => x["MaPKL"].ToString() == maPKL).FirstOrDefault()["MaDH"].ToString();
            LoadKHDongThung("KeHoachDongThungTonKho", madh, maPKL);
            searchLookUpEdit_MaPKL.EditValue = null;
        }
        #endregion
