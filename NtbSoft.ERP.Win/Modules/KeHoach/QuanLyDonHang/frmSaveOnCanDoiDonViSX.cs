        ...
        private void Save()
        {
            try
            {
                ...
                if (tblCanDoi?.Rows?.Count > 0)
                {
                    ...
                    List<string> lstLenhSX = lstSave.Select(x => x.MaLenh).Distinct().ToList();
                    ...
                    if (msResult.ToLower() == "true")
                    {
                        ...
                        {
                        foreach(string item in lstLenhSX)
                            {
                                string urlDB = $"{URL}SaveOnCanDoiDonViSX/DongBoQLSX?action=DongBo&para={item}";
