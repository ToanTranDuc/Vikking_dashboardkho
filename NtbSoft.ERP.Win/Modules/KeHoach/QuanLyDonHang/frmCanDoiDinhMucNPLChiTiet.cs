...
namespace NtbSoft.ERP.Win.Modules.QuanLyDonHang
{
    public partial class frmCanDoiDinhMucNPLChiTiet : DevExpress.XtraEditors.XtraForm
    {
        ...
        _madvsx = string.Empty, _malenhsanxuat = string.Empty, _tenlenh = string.Empty, _poid = string.Empty, _manpl = string.Empty, _magop = string.Empty, _sttlenh = string.Empty,_madhgop=string.Empty;
        ...
        public frmCanDoiDinhMucNPLChiTiet(string madh, string mahang, string tenhang, string malenh, string dot, string madvsx, string malenhsanxuat, string poid, int isKeoVe, string nguoitao, string magop, string tenlenh, string sttlenh,string madhgop="")
        {
            InitializeComponent();
        ...
        private void Them()
        {
            ...
            frmCanDoiDonHangChiTiet frm = new frmCanDoiDonHangChiTiet(true, _madh, _mahang, _tenhang, _dot, _poid, _malenh, _dot, _madvsx, _malenhsanxuat, _magop, _tenlenh, _sttlenh, _isKeoVe,_madhgop, SawDep);
            frm.ShowDialog();
            LoadData();
        }
            ...
            try
            {
                ...
                using (OfficeOpenXml.ExcelPackage excelPackage = new OfficeOpenXml.ExcelPackage(file))
                {
                    ...
                    foreach (DataRow item in tbl.Rows)
                    {
                        ...
                        worksheet.Cells[row, 3].Value = item["TenNhom"].ToString();
                        worksheet.Cells[row, 4].Value = item["TenTAVT"].ToString();
                        worksheet.Cells[row, 5].Value = item["TenVT"].ToString();
                    ...
                    ExcelRange borderRange = worksheet.Cells[29, 1, row - 1, 14]; // Từ dòng 29 đến dòng cuối cùng có dữ liệu
                ...
                using (OfficeOpenXml.ExcelPackage excelPackage = new OfficeOpenXml.ExcelPackage(file))
                {
                    ...
                    if (tblLSX != null && tblLSX.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow item in tblLSX.Rows)
                        {
                            ...
                            foreach (var lstSize in columnNamesWithSize)
                            {
                                worksheet.Cells[currentRow, colums].Value = Convert.ToInt32(item[lstSize]);
                    ...
                    {
                        // Title cho phần NPL
                        range = worksheet.Cells[currentRow, 1, currentRow, 14];
                        ...
                        worksheet.Cells[currentRow, 2].Value = "MÃ NPL";
                        worksheet.Cells[currentRow, 3].Value = "LOẠI VẬT TƯ";
                        worksheet.Cells[currentRow, 4].Value = "";
                        range = worksheet.Cells[currentRow, 5, currentRow, 6];
                        ...
                        worksheet.Cells[currentRow, 7].Value = "MÀU";
                        ...
                        worksheet.Cells[currentRow, 13].Value = "NHU CẦU";
                        worksheet.Cells[currentRow, 14].Value = "THỰC NHẬN";
                        worksheet.Cells[currentRow, 15].Value = "GHI CHÚ";
                        ...
                        foreach (DataRow item in tblNPL.Rows)
                        {
                            ...
                            worksheet.Cells[currentRow, 2].Value = item["MaVT"].ToString();
                           
                            worksheet.Cells[currentRow, 3].Value = item["TenNhom"].ToString();
                          
                            worksheet.Cells[currentRow, 4].Value = item["TenTAVT"].ToString();
                            range = worksheet.Cells[currentRow, 5, currentRow, 6];
                            ...
                            worksheet.Cells[currentRow, 15].Value = item["GhiChu"].ToString();
