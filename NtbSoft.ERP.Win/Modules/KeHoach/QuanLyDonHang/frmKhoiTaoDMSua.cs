...
{
    public partial class frmKhoiTaoDMSua : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmKhoiTaoDMSua(string makh = "", string mahang = "", string madot = "")
        {
            InitializeComponent();
        ...
        private void gVVatTu_ShowingEditor(object sender, CancelEventArgs e)
        {
            ...
            if (checkCD)
            {
                MessageBox.Show("Vật tư này đã được nhập cấp phát cho lệnh SX. Không thể sửa định mức.!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Question);
                e.Cancel = true;
            };
        }
        private bool CheckCanDoi(DataRow dr)
        {
            string urlCT = string.Format("{0}?para={1}&para1={2}&para2={3}&para3={4}&para4={5}", URL + $"KhoiTaoDM/CheckCanDoi",
                          dr["MaDot"].ToString(), dr["MaVTID"].ToString(), dr["MauVTID"].ToString(), dr["MaNhom"].ToString(), dr["KhoVaiID"].ToString());
            string jsonCT = Task.Run(async () => { return await _clientExtension.GetAsnyc(urlCT); }).Result;
