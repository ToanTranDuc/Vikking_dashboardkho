...
{
    public partial class frmNhapCapPhatDHTSua : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmNhapCapPhatDHTSua(string _magop, string _nguoitao, string _maLSX, string _tenKH, string _mahang, string _madh, string _malenh, string _soluong, string _tenhang, int _iskeove, string _dot)
        {
            this._magop = _magop;
            ...
            try
            {
                ...
                for (int i = 0; i < grv.RowCount; i++)
                {
                    ...
                    for (int j = 0; j < grv.Columns.Count; j++)
                    {
                        ...
                        if (column.FieldName == "MaVT" || column.FieldName == "TenVT" || column.FieldName == "KhoVai"
        ...
        public bool HasDuplicateRows(DataTable tblSave)
        {
            ...
            foreach (DataRow row in tblSave.Rows)
            {
                ...
                var key = $"{row["MaVT"]}_{row["MaMau"]}_{row["KhoVai"]}";
        ...
        {
            //string aaaas = checkEditCapPhat..ToString();
            //string aaaas2 = checkEditCapPhat.ValueChecked.ToString();
            //isCal = Convert.ToBoolean(checkEditCapPhat.ValueChecked);
        }
        ...
        private void barButtonItem1_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (repositoryItem != null)
            {
                ...
                if (editorValue != null)
                {
                    ...
                    if (dataSource != null)
                    {
                        // Tìm hàng trong DataTable có MaLenhSanXuat bằng editorValue
                        DataRow[] selectedRows = dataSource.Select($"MaLenhSanXuat = '{editorValue.ToString()}'");

                        if (selectedRows.Length > 0)
                        {
                            // Lấy hàng đầu tiên phù hợp (vì có thể có nhiều hàng)
        ...
        private void loadCapPhat(string madh, bool isGoc, string malenhsx = "")
        {
            ...
            if (isGoc)
            {
                ...
                // Lọc dữ liệu dựa vào cột IsNL
                //foreach (DataRow row in tblSave.Rows)
                //{
                //    if (row["IsNL"] != DBNull.Value && bool.TryParse(row["IsNL"].ToString(), out bool isNL))
                //    {
                //        if (isNL)
                //            tblNL.ImportRow(row);  // Thêm vào bảng tblNL nếu IsNL = true
                //        else
                //            tblPL.ImportRow(row); // Thêm vào bảng tblNPL nếu IsNL = false
                //    }
                //    else
                //    {
                //        // Nếu giá trị IsNL NULL hoặc không thể parse, mặc định thêm vào tblNL
                //        tblNL.ImportRow(row);
                //    }
                //}
