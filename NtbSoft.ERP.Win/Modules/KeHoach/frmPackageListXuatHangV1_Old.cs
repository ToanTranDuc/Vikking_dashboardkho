...
{
    public partial class frmPackageListXuatHangV1_Old : DevExpress.XtraEditors.XtraForm
    {
        ...
        private string _madh = string.Empty, _poid = string.Empty, _po = string.Empty, _maPKL = string.Empty, _maPKLXH = string.Empty, _maHang = string.Empty, _tenHang = string.Empty, _khachHang = string.Empty,
                _Cont = string.Empty, _oldCont = string.Empty, _Seal = string.Empty, _oldSeal = string.Empty, MaBooking = string.Empty,_maKH = string.Empty;
        ...
        public frmPackageListXuatHangV1_Old(string maPKLXH = "", string maDH = "",string maKH = "")
        {
            InitializeComponent();
        ...
        private void LoadDonHang()
        {
            try
            {
                string url = string.Format("{0}", URL + $"XuatHang/Get?Action=GetDH&Para1={searchLookUpEdit_KhachHang.EditValue.ToString()}&Para2=Para");
                string json = Task.Run(async () => { return await _clientExtension.GetAsnyc(url); }).Result;
        ...
        private void GetDataToPKL()
        {
            try
            {
                ...
                foreach (DataRow item in dtKHDT.Rows)
                {
                    ...
                    item["PO_XH"] = item["IsTon"].ToString() == "0" ? item["PO"] : item["PO_XH"];
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                foreach (DataColumn dc in dt.Columns)
                {
                    ...
                    for (int i = 0; i < slThung; i++)
                    {
                        ...
                        drNewRow["Cont"] = lstMaCont.FirstOrDefault(x => x.MaCont == dr["Cont"].ToString())?.TenCont;
