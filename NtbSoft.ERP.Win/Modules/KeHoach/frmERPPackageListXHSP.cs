...
namespace NtbSoft.ERP.Win.Modules.KeHoach
{
    public partial class frmERPPackageListXHSP : DevExpress.XtraEditors.XtraForm
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            LoadKhachHang();
            grvThungXuat.OptionsSelection.MultiSelect = true;
            grvThungXuat.OptionsSelection.MultiSelectMode = DevExpress.XtraGrid.Views.Grid.GridMultiSelectMode.RowSelect;
            grvThungXuat.CustomDrawRowIndicator += grvThungXuat_CustomDrawRowIndicator;
            grvChiTietThung.CustomDrawRowIndicator += grvThungXuat_CustomDrawRowIndicator;
            txtSLXuat11.Focus();
        }


        private void grvThungXuat_CustomDrawRowIndicator(object sender, DevExpress.XtraGrid.Views.Grid.RowIndicatorCustomDrawEventArgs e)
        {
        ...
        private void LoadData()
        {
            ...
            string url = string.Format("{0}", URL + $"KeHoachXuatHangThanhPham/Get?Action=DanhSachChiTietThung&Para1={_joinedMaDH}&Para2={_joinedGopPO}&Para3={_maPKL}");
            string json = Task.Run(async () => { return await _clientExtension.GetAsnyc(url); }).Result;
            ...
            {
                grcThungXuat.DataSource = tbl;
                grcChiTietThung.DataSource = tbl.Clone();
            }
        ...
        private async void LoadChiTietThung()
        {
            try
            {
                string url = string.Format("{0}", URL + $"KeHoachXuatHangThanhPham/Get?Action=GetPhieuXHEditNoPhieu&Para1={_joinedMaDH}&Para2={_joinedGopPO}&Para3={_maPKL}");

                string json = Task.Run(async () => { return await _clientExtension.GetAsnyc(url); }).Result;
                DataTable tblChiTiet = JsonConvert.DeserializeObject<DataTable>(json);
                DataTable tblThungXuat = grcThungXuat.DataSource as DataTable;
                if (tblChiTiet == null || tblChiTiet.Rows.Count == 0)
                {
                    grcChiTietThung.DataSource = tblThungXuat == null ? null : tblThungXuat.Clone();
                    return;
                }
        ...
        private void DeductExportedQuantity(DataTable tblChiTiet, DataTable tblDaXuatFull)
        {
            foreach (DataRow row in tblChiTiet.Rows)
            {
                ...
                string sizeID = row["SizeID"]?.ToString() ?? string.Empty;
                ...
                DataRow rowDaXuat = tblDaXuatFull.AsEnumerable().FirstOrDefault(r =>
                    ...
                    (r.Field<string>("sizeID") ?? string.Empty) == maPKL &&
        ...
        private void ProcessExportRow(DataRow sourceRow, DataTable targetTable)
        {
            ...
            string sizeID = sourceRow["SizeID"]?.ToString() ?? string.Empty;
            ...
            DataRow existingRow = targetTable.AsEnumerable().FirstOrDefault(t =>
                ...
                (t.Field<string>("SizeID") ?? string.Empty) == sizeID &&
        ...
        private void ResetAndRecalculateSLSPCL(DataTable tblChiTiet, DataTable tblDaXuatFull)
        {
            ...
            foreach (DataRow row in tblChiTiet.Rows)
            {
                ...
                if (rowDaXuat != null)
                {
                    ...
                    int soLuongGoc = row["SLSPCL"] != DBNull.Value
                        ? Convert.ToInt32(row["SLSPCL"])
                        : 0;
        ...
        private void simpleButton3_Click(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            GetDataThung();
        }

        private void simpleButton1_Click(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ResetSoLuong();
        }

        private void simpleButton2_Click(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            GetSoLuong();
        }

        private void btnChuyenThung_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
          

            int[] selectedHandles = grvThungXuat.GetSelectedRows();
            ...
            foreach (int handle in selectedHandles)
            {
                DataRow row = grvThungXuat.GetDataRow(handle);
                if (row == null) continue;

                listBarcode.Add(row["QRCode"].ToString());
            }
            DataTable tbl = grcThungXuat.DataSource as DataTable;
            if (tbl == null) return;

            foreach (string barcode in listBarcode)
            {
                DataRow row = tbl.AsEnumerable()
                                 .FirstOrDefault(r => r["QRCode"].ToString() == barcode);

                if (row == null) continue;

                ChuyenThungBarcode(barcode);
            }
            grvThungXuat.ClearSelection();
        }
        private bool CheckExistsBarCode(string barcode)
        {
            DataTable tbl = grcThungXuat.DataSource as DataTable;
            bool exists = tbl.AsEnumerable()
                             .Any(r => r["QRCode"].ToString() == barcode);
            if (exists)
                return true;
            else
                return false;
        }
        private void ChuyenThungBarcode(string barcode)
        {
            DataTable tbl = grcThungXuat.DataSource as DataTable;
            DataTable tblChiTietThung = grcChiTietThung.DataSource as DataTable;

            DataRow rowBarcode = tbl.AsEnumerable()
                                    .First(x => x["QRCode"].ToString() == barcode);

            DataRow newRow = tblChiTietThung.NewRow();

            foreach (DataColumn col in tblChiTietThung.Columns)
            {
                if (tbl.Columns.Contains(col.ColumnName))
                {
                    newRow[col.ColumnName] = rowBarcode[col.ColumnName];
                }
            }
            tblChiTietThung.Rows.Add(newRow);
            tbl.Rows.Remove(rowBarcode);


        }

        private void txtSLXuat11_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                string barcode = txtSLXuat11.Text.Trim();
                BarCodeHandel(barcode);
                txtSLXuat11.Text = "";
                txtSLXuat11.Focus();
            }
        }
        private string BarCodePhieu(string barcode)
        {
            string url = string.Format("{0}", URL + $"KeHoachXuatHangThanhPham/Get?Action=GetBarCodePhieu&Para1={barcode}");
            string json = Task.Run(async () => { return await _clientExtension.GetAsnyc(url); }).Result;
            DataTable tblBarcode = JsonConvert.DeserializeObject<DataTable>(json);

            if (tblBarcode == null || tblBarcode.Rows.Count == 0) return "";
            return tblBarcode.Rows[0]["PhieuXH"].ToString();



        }
        private void BarCodeHandel(string barcode)
        {
            string url = string.Format("{0}", URL + $"KeHoachXuatHangThanhPham/Get?Action=GetBarCode&Para1={barcode}");

            string json = Task.Run(async () => { return await _clientExtension.GetAsnyc(url); }).Result;
            DataTable tblBarcode = JsonConvert.DeserializeObject<DataTable>(json);
            if (tblBarcode == null || tblBarcode.Rows.Count == 0)
            {
                XtraMessageBox.Show($"Barcode không tồn tại trong PKL", "Cảnh báo",
                   MessageBoxButtons.OK, MessageBoxIcon.Warning);
                SystemSounds.Hand.Play();
                return;
            }
            string joinDH = string.Join(";", grvSDH.GetSelectedRows().Select(rowHandle2 =>
                    grvSDH.GetRowCellValue(rowHandle2, searchLookUpEditMaDH.Properties.ValueMember)));
            string joinpo = string.Join(";", grvSPO.GetSelectedRows().Select(rowHandle2 =>
                  grvSPO.GetRowCellValue(rowHandle2, searchLookUpEditPO.Properties.ValueMember)));
            string[] parsMaDH = joinDH.Split(';');
            string[] parsPOID = joinpo.Split(';');
            string[] parsMaMaPKL = _maPKL.Split(';');
            string maKH = (searchLookUpEdit1.EditValue as string);
            string madh = tblBarcode.Rows[0]["MaDH"].ToString();
            string poid = tblBarcode.Rows[0]["POEdit"].ToString();
            string makhedit = tblBarcode.Rows[0]["MaKH"].ToString();
            string maPKL = tblBarcode.Rows[0]["MaPKL"].ToString();
            bool checkFirst = checkPO == 0 ? true : false;
            if (maKH != makhedit)
            {
                _checkEdit = true;
                _maDHEdit = madh;
                _maKHEdit = makhedit;
                _maPKLEdit = tblBarcode.Rows[0]["MaPKLEdit"].ToString();
                _POEdit = poid;
                LoadKhachHang();
                SystemSounds.Asterisk.Play();
            }
            else if (!parsMaDH.Contains(madh))
            {
                _checkEdit = true;
                _maDHEdit = madh;
                _maKHEdit = makhedit;
            ...
            else if (!parsPOID.Contains(poid))
            {
                _checkEdit = true;
            ...
            else if (!parsMaMaPKL.Contains(maPKL))
            {
                _checkEdit = true;
            ...
            {
                bool checkBarcode = CheckExistsBarCode(barcode);
                if (!checkBarcode)
                {
                    string barcodePhieu = BarCodePhieu(barcode);
                    if(barcodePhieu == "")
                    {
                        XtraMessageBox.Show($"Barcode {barcode} không tồn tại trong danh sách!", "Cảnh báo",
                       MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    else
                    {
                        XtraMessageBox.Show($"Barcode {barcode} đã được xuất ở PXH {barcodePhieu}!", "Cảnh báo",
                      MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
            ...
            if (!parsMaDH.Contains(madh) && !checkFirst)
            {
                searchLookUpEditMaDH.ShowPopup();
                searchLookUpEditMaDH.ClosePopup();
                searchLookUpEditPO.ShowPopup();
            ...
            else if (!parsPOID.Contains(poid) && !checkFirst)
            {
                searchLookUpEditPO.ShowPopup();
                searchLookUpEditPO.ClosePopup();
                searchLookUpEditMaPKL.ShowPopup();
            ...
            else if (!parsMaMaPKL.Contains(poid) && !checkFirst)
            {
                searchLookUpEditMaPKL.ShowPopup();
                searchLookUpEditMaPKL.ClosePopup();
            }
