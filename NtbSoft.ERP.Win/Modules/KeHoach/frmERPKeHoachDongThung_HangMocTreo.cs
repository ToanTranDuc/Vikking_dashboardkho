...
{
    public partial class frmERPKeHoachDongThung_HangMocTreo : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmERPKeHoachDongThung_HangMocTreo(string MaDH, string MaDHDisplay, string MaHang, string TenHang, string POID, string MaPKL)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                foreach (var itemPO in _dttempPO)
                {
                    ...
                    foreach (var itemDVSX in _dtdistinctDVSX)
                    {
                        ...
                        var dtSourceTL = grcTiLe.DataSource as DataTable;
                        foreach (DataRow dr in dtSourceTL.Rows)
                        {
                            ...
                            _dr["DauSize"] = dtSize.AsEnumerable().Where(x => x["SizeTypeID"].ToString() == dr["DauSizeID"].ToString()).FirstOrDefault()["SizeType"];
                            _dr["DauSizeID"] = dr["DauSizeID"];
                            _dr["TenMau"] = dtSize.AsEnumerable().Where(x => x["ColorID"].ToString() == dr["ColorID"].ToString()).FirstOrDefault()["TenMau"];
                            _dr["ColorID"] = dr["ColorID"];
                            ...
                            _dr["KieuLap"] = "0";
                            foreach (DataRow drSize in _dtTemp.Rows)
                            {
                            ...
                            bool IsThieu = false;
                            foreach (DataColumn dc in dtSourceTL.Columns)
                            {
                                ...
                                var _dtTempA = _dtdata.AsEnumerable().Where(x => x["SizeTypeID"].ToString() == dr["DauSizeID"].ToString()
                                                                                      && x["ColorID"].ToString() == dr["ColorID"].ToString()
                                                                                      && x["SizeID"].ToString() == SizeID);
                            ...
                            _drAdd["TrongLuongA"] = TLThungChan;
        ...
        private void BindingDaSizeDaTiLe()
        {
            ...
            var _lstMau = dtSize.AsEnumerable().Select(x => new
            {
                ColorID = x["ColorID"].ToString(),
                TenMau = x["TenMau"].ToString()
            }).Distinct().ToList();
            string json = JsonConvert.SerializeObject(_lstMau);
            DataTable _dtColor = JsonConvert.DeserializeObject<DataTable>(json);

            var _lstDaSize = dtSize.AsEnumerable().Select(x => new
            {
                DauSizeID = x["SizeTypeID"].ToString(),
                DauSize = x["SizeType"].ToString()
            }).Distinct().ToList();
            string jsonA = JsonConvert.SerializeObject(_lstDaSize);
            DataTable _dtDauSize = JsonConvert.DeserializeObject<DataTable>(jsonA);

            DataTable dtSourceTemp = new DataTable();
            grvInit.Columns.Clear();
            grvTiLe.Columns.Clear();
            dtSourceTemp.Columns.Add("DauSizeID", typeof(string));
            dtSourceTemp.Columns.Add("ColorID", typeof(string));
            dtSourceTemp.Columns.Add("SLThung", typeof(int));
            CreateRepoSearchDaTiLe("DauSize", "InSeam", "DauSizeID", _dtDauSize);
            CreateRepoSearchDaTiLe("Color", "Màu", "ColorID", _dtColor);
            ...
            grcInit.DataSource = dtSourceTemp;

        }
        private void CreateRepoSearchDaTiLe(string type, string caption, string fieldname, DataTable dt)
        {
            GridColumn col = new GridColumn();
            col.Caption = caption;
            ...
            col.AppearanceHeader.BackColor = Color.AliceBlue;
            col.Visible = true;
            RepositoryItemSearchLookUpEdit repoSearch = new RepositoryItemSearchLookUpEdit();
            repoSearch.DataSource = dt;
            if (type == "Color")
            {
                repoSearch.DisplayMember = "TenMau";
                repoSearch.ValueMember = "ColorID";
            }
            else if (type == "DauSize")
            {
                repoSearch.DisplayMember = "DauSize";
                repoSearch.ValueMember = "DauSizeID";
            }

            GridView popupView = repoSearch.View;

            if (type == "Color") popupView.Columns.AddVisible("TenMau", caption);
            if (type == "DauSize") popupView.Columns.AddVisible("DauSize", caption);
            grcInit.RepositoryItems.Add(repoSearch);
            grvInit.Columns.Add(col);
            grvInit.Columns[fieldname].ColumnEdit = repoSearch;


            GridColumn colTL = new GridColumn();
            colTL.Caption = caption;
            colTL.FieldName = fieldname;
            colTL.AppearanceHeader.BackColor = Color.AliceBlue;
            colTL.Visible = true;
            RepositoryItemSearchLookUpEdit repoSearchTL = new RepositoryItemSearchLookUpEdit();
            repoSearchTL.DataSource = dt;
            if (type == "Color")
            {
                repoSearchTL.DisplayMember = "TenMau";
                repoSearchTL.ValueMember = "ColorID";
            }
            else if (type == "DauSize")
            {
                repoSearchTL.DisplayMember = "DauSize";
                repoSearchTL.ValueMember = "DauSizeID";
            }

            GridView popupViewTL = repoSearchTL.View;
            if (type == "Color") popupViewTL.Columns.AddVisible("TenMau", caption);
            else if (type == "DauSize") popupViewTL.Columns.AddVisible("DauSize", caption);

            grcTiLe.RepositoryItems.Add(repoSearchTL);
            grvTiLe.Columns.Add(colTL);
            grvTiLe.Columns[fieldname].ColumnEdit = repoSearchTL;
            // comboBox.Items.AddRange(new DataTable());
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                var dtSource = grcInit.DataSource as DataTable;
                ...
                else
                {
                    ...
                    {
                        if (dc.ColumnName == "DauSizeID" || dc.ColumnName == "ColorID") continue;
        ...
        private void gridView3_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            {
                DataRow dr = grvInit.GetFocusedDataRow();
            ...
            {
                DataTable dt = grcInit.DataSource as DataTable;
