...
{
    public partial class frmPackageListTraHang : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmPackageListTraHang(string maPKLXH = "")
        {
            InitializeComponent();
        ...
        private void SaveXuatHang()
        {
            try
            {
                ...
                foreach (var itemMaDH in distinctDH)
                {
                    ...
                    {
                        int carton = _dtTempData.AsEnumerable().Where(y => y["MaDH"].ToString() == itemMaDH.MaDH && y["POID"].ToString() == itemPO.POID).Sum(x => Convert.ToInt32(x["SLThung"]));
                        int SLPCS = _dtTempData.AsEnumerable().Where(y => y["MaDH"].ToString() == itemMaDH.MaDH && y["POID"].ToString() == itemPO.POID).Sum(x => Convert.ToInt32(x["TotalPiece"]));
