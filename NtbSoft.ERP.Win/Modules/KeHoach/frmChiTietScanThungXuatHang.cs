...
{
    public partial class frmChiTietScanThungXuatHang : DevExpress.XtraEditors.XtraForm
    {
        ...
        private bool flagInitA = false;
        private bool isStartFillBarcode = false;
        private int RowHandle = 0;
        private bool flagSMSA = false;
        private DataTable dtDataT = new DataTable();

        public frmChiTietScanThungXuatHang(DataRow dr, bool flagAllowEdit = true, bool flagInit = false, bool flagScan = false,bool flagSMS = false)
        {
            InitializeComponent();
            URL = (string)settingsReader.GetValue("URL", typeof(String));
            _clientExtension = new HttpClientExtension();
            drGetData = dr;
            MaPKL_XH = drGetData["MaPKL_XH"].ToString();
            Cont = drGetData["Cont"].ToString();
            MaDH = drGetData["MaDH"].ToString();
            POID = flagSMS ? "All" : drGetData["POID"].ToString();
            MaPKL = drGetData["MaPKL"].ToString();
            MaLenh = drGetData["MaLenh"].ToString();
        ...
        {
            string url = string.Format("{0}", URL + $"ScanBarcode/Get?Action=GetDSThungPKL&Para1={MaPKL_XH}&Para2={Cont}&Para3={MaDH}&Para4={POID}" +
