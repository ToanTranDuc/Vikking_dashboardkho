...
{
    public partial class frmPOMau : Form
    {
        ...
        {
            string urlID = string.Format("{0}", URL + "POMau/Get?action=GetSTT");
            ...
            string newPhieuNumber = GeneratePhieuNumber(Convert.ToInt32(tbl.Rows[0][0]));
            textBoxMP.Text = newPhieuNumber;
            CreateSearchLookup();
            loadRepoChungLoai();
        ...
        private void Luu_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (textEditForecastqty.EditValue == null || string.IsNullOrWhiteSpace(textEditForecastqty.Text))
            {
                XtraMessageBox.Show("Vui lòng nhập Forecast qty!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            if (string.IsNullOrWhiteSpace(textBoxColorways.Text))
            {
                XtraMessageBox.Show("Vui lòng nhập # Of Colorways!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            if (string.IsNullOrWhiteSpace(textBoxSizeRatio.Text))
            {
                XtraMessageBox.Show("Vui lòng nhập Size Ratio!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            //if (string.IsNullOrWhiteSpace(textBoxPosition.Text))
            //{
            //    XtraMessageBox.Show("Vui lòng nhập Position!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            //    return;
            //}
            if (searchLookUpEditQuotainsize.EditValue == null || string.IsNullOrWhiteSpace(searchLookUpEditQuotainsize.Text))
            {
                XtraMessageBox.Show("Vui lòng chọn Quotation size!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            {
                //XtraMessageBox.Show("Vui lòng thêm ít nhất một vật tư!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                //return;
            }

            //var requiredColumns = new[]
            //{
            //    new { Field = "ChungLoaiVatTu", Label = "Chủng loại vật tư" },
            //    new { Field = "MaVT",          Label = "Item Code" },
            //    new { Field = "ChiTiet",       Label = "Mô tả" },
            //    new { Field = "KhoVai",        Label = "Khổ/Size" },
            //    new { Field = "MauVT",         Label = "Màu vật tư" },
            //    new { Field = "MaMauVT",       Label = "Mã màu vật tư" },
            //};
            ...
            //    {
            //        if (string.IsNullOrWhiteSpace(dr[col.Field]?.ToString()))
            //        {
            //            XtraMessageBox.Show(
            //                $"Dòng {i + 1}: Vui lòng điền {col.Label}!",
            //                "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            //            int rowHandle = gridView1.GetRowHandle(i);
            //            gridView1.FocusedRowHandle = rowHandle;
            //            gridView1.FocusedColumn = gridView1.Columns[col.Field];
            ...
            if (tbl != null && tbl.Rows.Count > 0)
            {
                foreach (DataRow dr in tbl.Rows)
                {
                    ...
                    _drow["MaPhieu"] = formattedPhieuNumber;
            ...
            {
                if (dtSaveCT != null && dtSaveCT.Rows.Count > 0)
                {
        ...
        {
            if (gridView1.FocusedColumn?.FieldName == "ChiTiet")
            {
                DataRow row = gridView1.GetFocusedDataRow();
                if (row == null) return;
        ...
        private async void btnAddPicture_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                ...
                try
                {
                    ...
                    {
                        XtraMessageBox.Show("Không đọc được ảnh!");
                        return;
                    }
                    _zoomFactor = 1f;
                    _panOffset = Point.Empty;
