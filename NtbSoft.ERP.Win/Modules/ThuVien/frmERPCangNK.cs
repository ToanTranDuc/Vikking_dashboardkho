...
{
    public partial class frmERPCangNK : DevExpress.XtraEditors.XtraForm
    {
        ...
        public bool KiemTraVaCanhBao(DataTable dt)
        {
            ...
            for (int i = dt.Rows.Count - 1; i >= 0; i--)
            {
                ...
                string tenKH = row["TenCang"]?.ToString().Trim();
            ...
            try
            {
                ...
                if (duplicateRows.Any())
                {
                    ...
                    foreach (var row in duplicateRows)
                    {
                        ...
                        message += $"- Dòng {rowIndex}: {row["TenCang"]}\n";
                    }
                ...
                {
                    if (string.IsNullOrWhiteSpace(dr["TenCang"].ToString()))
                    {
                       
                        XtraMessageBox.Show("Vui lòng điền thông tin Cảng", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                }

                clsWriteLogThuVienLib.WriteLog(URL, _clientExtension, lstLog, "ERP_CangNK");
                string url = $"{URL}ERPThuVienNK/PostCang?Action=POSTCANG";
        ...
        private void btnXoa_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                if (dr == null) return;
                string macang = dr["MaCang"].ToString();
                string urlCheck = $"{URL}ERPThuVienNK/Get?Action=GETCHECKCANG&para={macang}";
                string jsonCheck = Task.Run(async () => { return await _clientExtension.GetAsnyc(urlCheck); }).Result;
                if (jsonCheck != "[]")
                {
                    XtraMessageBox.Show("Cảng này đã được sử dụng. Không thể xóa.!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                if (messResult == DialogResult.Yes)
                {
                    ...
                    if (dr["ID"].ToString() != "0")
                    {
                        string url = $"{URL}ERPThuVienNK/Delete?Action=DELETECANG&para={dr["ID"]}&para2={GlobleData.UserName}";
        ...
        private List<DataRow> GetDuplicateRows(DataTable dt)
        {
            ...
            var groups = dt.AsEnumerable()
              .GroupBy(row => new
              {
                  TenCang = ReplaceSpecialCharacters(RemoveVietnameseTone(row.Field<string>("TenCang")))
              })
              .Where(g => g.Count() > 1);
        ...
        private bool checkDup(DataTable dt)
        {
            ...
            var duplicates = dt.AsEnumerable()
                .GroupBy(row => new
                {
                    TenKH = ReplaceSpecialCharacters(RemoveVietnameseTone(row.Field<string>("TenCang"))),
