    ...
    public partial class frmBrand : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void LuuDong()
        {
            try
            {
                ...
                if (row == null)
                {
                    XtraMessageBox.Show(Resources.UpdateDataBefore, Resources.Warning,MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                if (string.IsNullOrWhiteSpace(row.TenBrand))
                {
                    MessageBox.Show("Vui lòng nhập tên Brand.", "Thông báo",MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                lstRowUpdate = lstRowUpdate.Distinct().OrderBy(i => i).ToList();
                var lstUpdate = new List<BrandEntity>();
                foreach (int idx in lstRowUpdate)
                {
                    BrandEntity item = gridViewBrand.GetRow(idx) as BrandEntity;
                    if (item != null && !string.IsNullOrWhiteSpace(item.TenBrand))
                    {
                        string currentUser = GlobleData.UserName;
                        if (!string.IsNullOrEmpty(currentUser))
                        {
                ...
                var tenBrandList = lstBrandEntity
                    .Where(x => !lstUpdate.Any(u => u.MaBrand == x.MaBrand))
                    .Select(x => (x.TenBrand ?? "").Trim().ToUpper())
                    .ToHashSet();
                foreach (var item in lstUpdate)
                {
                    ...
                    if (isDuplicate)
                    {
                        MessageBox.Show($"Tên {item.TenBrand} đã tồn tại. Vui lòng nhập tên khác!",
                            "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                }
            ...
            {
                XtraMessageBox.Show("Lưu đã xảy ra lỗi. Vui lòng kiểm tra lại!","Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
