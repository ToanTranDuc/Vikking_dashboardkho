...
{
    public partial class frmCanDoiNPLDH : DevExpress.XtraEditors.XtraForm
    {
        ...
        private bool _preventClose1 = false;
        private bool _preventClose2 = false;
        ...
        public frmCanDoiNPLDH(string nplID = "")
        {
            InitializeComponent();
        ...
        private void getVatTuToUpdate()
        {
            ...
            {
                var request = XuLyVTRequestPost.createDefault("CanDoiNPL");
                request.Action = "getvattucancandoieditDHVT";

                if (phieucandoiTable != null && phieucandoiTable.Rows.Count > 0)
                {
                    var distinctMaDH = phieucandoiTable.AsEnumerable()
                        .Select(r => r["MaDH"].ToString())
                        .Distinct()
                        .ToList();

                    foreach (string maDH in distinctMaDH)
                    {
                        DataRow newRow = request.TypeTable.NewRow();
                        newRow["MaDH"] = maDH;
                        newRow["MaPhieu"] = _nplID;
                        request.TypeTable.Rows.Add(newRow);
                    }
                }
        ...
        private void loadSearchLookupEdit1()
        {
            ...
            view.PopupMenuShowing += (s, ex) =>
            {
                ...
                ex.Allow = false; // tắt menu mặc định
                _preventClose1 = true;
                ...
                menu.Closed += (ms, me) => { _preventClose1 = false; };
        ...
        private void searchLookUpEdit1_CloseUp(object sender, DevExpress.XtraEditors.Controls.CloseUpEventArgs e)
        {
            ...
            if (!isSame)
            {
                ...
                resetVatTuDaChon();
                slTonKhoText.EditValue = null;
                ngayDBDKEdit.EditValue = null;
            }
        }
        ...
        private void loadSearchLookupEdit2()
        {
            ...
            GridColumn colIsChon2 = view.Columns["IsChon"];
            if (colIsChon2 != null)
            {
                colIsChon2.Caption = "Chọn";
                colIsChon2.Visible = true;
                colIsChon2.VisibleIndex = 0;
                colIsChon2.Width = 60;
                colIsChon2.OptionsColumn.AllowEdit = true;
                colIsChon2.OptionsColumn.FixedWidth = true;
                colIsChon2.ColumnEdit = new RepositoryItemCheckEdit();
            }
            ...
            view.PopupMenuShowing += (s, ex) =>
            {
                ...
                _preventClose2 = true;
                ContextMenuStrip menu = new ContextMenuStrip();
                menu.Items.Add("Chọn tất cả", null, (ms, me) => { foreach (DataRow r in dt.Rows) r["IsChon"] = true; view.RefreshData(); });
                menu.Items.Add("Bỏ chọn tất cả", null, (ms, me) => { foreach (DataRow r in dt.Rows) r["IsChon"] = false; view.RefreshData(); });
                menu.Closed += (ms, me) => { _preventClose2 = false; };
                menu.Show(Cursor.Position);
            };

            searchLookUpEdit2.QueryCloseUp += (s, ex) =>
            {
                if (_preventClose2)
                {
                    ex.Cancel = true;
                    _preventClose2 = false;
                    return;
                }
                ex.Cancel = true;
            };
        ...
        {
            //DataTable tblSource = searchLookUpEdit2.Properties.DataSource as DataTable;
            //if (tblSource == null) return;

            //List<DataRow> checkedRows = tblSource.AsEnumerable()
            //    .Where(r => r["IsChon"] != DBNull.Value && (bool)r["IsChon"] == true)
            //    .ToList();

            //if (checkedRows.Count == 0) return;

            //vattuRow = checkedRows.Last();
            ////if (vattuRow == null) return;
        ...
        private void searchLookUpEdit2_CloseUp(object sender, DevExpress.XtraEditors.Controls.CloseUpEventArgs e)
        {
            ...
            vattudachon["MaMauVT"] = vattuRow["MaMauVT"].ToString();
            vattudachon["MaKhoVT"] = vattuRow["MaKhoVT"].ToString();
            //searchLookUpEdit2.EditValue = vattuRow["MaVTID"];
            slTonKho = XuLyVTUnits.SmartTryParse<decimal>(vattuRow["TonKho"]);
        ...
        private void loadGridControl1()
        {
            ...
            gridView1.Columns["TonKho"].ColumnEdit = textEdit;
        ...
        private void getCanDoiVTTable()
        {
            ...
            if (vattuRow == null) return;
            List<DataRow> checkRows = vattuTable.AsEnumerable()
                .Where(r => r["IsChon"] != DBNull.Value && (bool)r["IsChon"] == true).ToList();
            if (checkRows.Count == 0) return;
            try
            {
                var request = XuLyVTRequestPost.createDefault("CanDoiNPL");
                ...
                //reqRow["MaMauVT"] = vattuRow["MaMauVT"].ToString();
                //reqRow["MaKhoVT"] = vattuRow["MaKhoVT"].ToString();
                foreach (DataRow vtRow in checkRows)
                {
                    DataRow reqRow = request.TypeTable.NewRow();
                    reqRow["MaVTID"] = vtRow["MaVTID"].ToString();
                    reqRow["MaNhomVT"] = vtRow["MaNhomVT"].ToString();
                    reqRow["MaMauVT"] = vtRow["MaMauVT"].ToString();
                    reqRow["MaKhoVT"] = vtRow["MaKhoVT"].ToString();
                    request.TypeTable.Rows.Add(reqRow);
                }
                //request.TypeTable.Rows.Add(reqRow);
                string urlGetListDataTable = URL + "CanDoiNPL/GetByTypeTable";
                ...
                if (!candoiVTTable.Columns.Contains("TyLeMuaThem"))
                    candoiVTTable.Columns.Add("TyLeMuaThem", typeof(decimal));
                if (!candoiVTTable.Columns.Contains("Sort"))
                    candoiVTTable.Columns.Add("Sort", typeof(int));
                if (!candoiVTTable.Columns.Contains("STTDH"))
                    candoiVTTable.Columns.Add("STTDH", typeof(int));
                foreach (DataRow row in candoiVTTable.Rows)
                {
                    row["generatedID"] = XuLyVTUnits.generatedTimeKey("id");
                }
        ...
        private void saveCanDoiNPL()
        {
            ...
            string key = XuLyVTUnits.generatedTimeKey("group_dhvt");
            int sttdh = 1;
        ...
        private void updateCanDoiNPL()
        {
            ...
            for (int i = 0; i < gridView1.RowCount; i++)
            {
                ...
                sortedRows.Add(gridRow);
            }

            // Gán Sort sau khi đã thu thập xong
            for (int i = 0; i < sortedRows.Count; i++)
            {
                sortedRows[i]["Sort"] = i + 1;
            }
            ...
            foreach (DataRow gridRow in sortedRows)
            {
                DataRow row = request.TypeTable.NewRow();
                foreach (DataColumn col in request.TypeTable.Columns)
                {
                    row[col.ColumnName] = dt.Columns.Contains(col.ColumnName)
                        ? gridRow[col.ColumnName]
                        : DBNull.Value;
                }
        ...
        private void upBtn_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (rowHandle > 0)
            {
                ...
                // Lưu lại Sort và SLCanDoiKho trước khi swap
                object sortRow = row["Sort"];
                object sortPrevRow = prevRow["Sort"];
                decimal slCanDoiRow = XuLyVTUnits.SmartTryParse<decimal>(row["SLCanDoiKho"].ToString());
                decimal slCanDoiPrevRow = XuLyVTUnits.SmartTryParse<decimal>(prevRow["SLCanDoiKho"].ToString());

                object[] temp = row.ItemArray;
                row.ItemArray = prevRow.ItemArray;
                prevRow.ItemArray = temp;

                // Gán lại Sort và SLCanDoiKho đúng vị trí
                row["Sort"] = sortRow;
                prevRow["Sort"] = sortPrevRow;
                row["SLCanDoiKho"] = slCanDoiPrevRow;
                prevRow["SLCanDoiKho"] = slCanDoiRow;
        ...
        private void downBtn_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (rowHandle < gridView1.RowCount - 1)
            {
                ...
                // Lưu lại Sort và SLCanDoiKho trước khi swap
                object sortRow = row["Sort"];
                object sortNextRow = nextRow["Sort"];
                decimal slCanDoiRow = XuLyVTUnits.SmartTryParse<decimal>(row["SLCanDoiKho"].ToString());
                decimal slCanDoiNextRow = XuLyVTUnits.SmartTryParse<decimal>(nextRow["SLCanDoiKho"].ToString());
                ...
                // Gán lại Sort và SLCanDoiKho đúng vị trí
                row["Sort"] = sortRow;
                nextRow["Sort"] = sortNextRow;
                row["SLCanDoiKho"] = slCanDoiNextRow;
                nextRow["SLCanDoiKho"] = slCanDoiRow;
        ...
        //            if (!updown)
        //            {
        //                decimal slTonKhoGoc = XuLyVTUnits.SmartTryParse<decimal>(group.First()["SLTonKhoSauCanDoi"].ToString())
        //                                    + XuLyVTUnits.SmartTryParse<decimal>(group.First()["SLCanDoiKho"].ToString());
        //                foreach (DataRow r in group)
        //                {
        //                    r["TonKhoCu"] = slTonKhoGoc;
        //                    r["TonKho"] = slTonKhoGoc;
        //                }
        //            }
        ...
        //                row["TonKho"] = XuLyVTUnits.SmartTryParse<decimal>(row["TonKhoCu"].ToString());
        ...
        //    {
        //        int sort = 1;
        //        var groupedByVatTu = gridControl1Table.AsEnumerable()
        //            .GroupBy(r => r["MaVTID"]?.ToString() ?? "");

        //        foreach (var group in groupedByVatTu)
        //        {
        //            decimal slTonKhoVT = XuLyVTUnits.SmartTryParse<decimal>(
        //                group.First()["TonKho"].ToString());
        ...
        private void calcCanDoiNPL(bool updown)
        {
            ...
            if (isUpdate)
            {
                if (!updown)
                {
                    ...
                    foreach (var group in groupedByVatTu)
                    {
                        ...
                        decimal slTonKhoVT = slTonKhoGoc;
                        foreach (DataRow row in group)
                        {
            ...
            else
            {
                ...
                foreach (var group in groupedByVatTu)
                {
                    decimal slTonKhoVT = XuLyVTUnits.SmartTryParse<decimal>(group.First()["TonKho"].ToString());
        ...
        private void searchLookUpEdit2_CustomDisplayText(object sender, DevExpress.XtraEditors.Controls.CustomDisplayTextEventArgs e)
        {
            ...
            e.DisplayText = string.Join(";", checkedRows.Select(r => r["ChiTiet"]?.ToString() ?? ""));
        }
        ...
        private void gridView1_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            // Lấy danh sách row theo thứ tự hiển thị
            ...
            string maVTIDEdited = editedRow["MaVTID"]?.ToString() ?? "";

            List<DataRow> groupRows = orderedRows
                .Where(r => (r["MaVTID"]?.ToString() ?? "") == maVTIDEdited)
                .ToList();

            int editedIndex = groupRows.IndexOf(editedRow);
            if (editedIndex < 0) return;
            decimal tonKhoCurrent;
            if (editedIndex == 0)
            {
                tonKhoCurrent = XuLyVTUnits.SmartTryParse<decimal>(groupRows[0]["TonKhoCu"]?.ToString() ?? "0");
            }
            else
            {
                DataRow prevRow = groupRows[editedIndex - 1];
                tonKhoCurrent = XuLyVTUnits.SmartTryParse<decimal>(prevRow["SLTonKhoSauCanDoi"].ToString());
            }
            for (int i = editedIndex; i < groupRows.Count; i++)
            {
                DataRow row = groupRows[i];
                ...
                if (i == editedIndex && e.Column.FieldName == "SLCanDoiKho")
                {
                    ...
                    if (slCanDoi > slNhuCau) slCanDoi = slNhuCau;
