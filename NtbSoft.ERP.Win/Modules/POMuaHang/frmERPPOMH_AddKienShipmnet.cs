...
{
    public partial class frmERPPOMH_AddKienShipmnet : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmERPPOMH_AddKienShipmnet(DataRow dr, DataTable _dt, string solo = "", bool IsNPL = false, decimal tileNW = 0m, decimal tileGW = 0m, string madvcd = "", string tendvcd = "", string soloid = "", string batch = "", string pomua = "")
        {
            InitializeComponent();
        ...
        private void loadDonVi()
        {
            ...
            string url = $"{URL}ERPNhapKhoNPL/GET?Action=GETDVCD&para={_drRow["DVTT"]}";
        ...
        private bool XacNhan()
        {
            try
            {
                ...
                var donVi = string.IsNullOrEmpty(searchLookUpEdit1.Text.ToString()) ? _drRow["DVTT"].ToString() : searchLookUpEdit1.Text.ToString();

                string maVTID = _drRow["MaVTID"].ToString();
                string maMauVT = _drRow["MauVTID"].ToString();
                string khoVaiID = _drRow["KhoVaiID"].ToString();
                string MaCLVT = _drRow["MaCLVT"].ToString();
                string maVatTu_Ghep = _drRow["MaVTGhep"].ToString();

                string soLoID = _soloid;

                string maNPL = RemoveVietnameseTone(ReplaceSpecialCharacters(MaCLVT) + "@" + ReplaceSpecialCharacters(maVTID) + "@" + ReplaceSpecialCharacters(maMauVT) + "@" + ReplaceSpecialCharacters(khoVaiID));
                ...
                for (int i = 0; i < countKien; i++)
                {
                  ...
                  /*  nr["ThanhTien"] = 0;*/
                    nr["SoLuongThucTe"] = soLuongTheoCT;
                    //nr["Pallet"] = "";
