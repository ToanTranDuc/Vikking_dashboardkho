...
{
    public partial class frmPhieuMuaHangPCDDH : DevExpress.XtraEditors.XtraForm
    {
        ...
        string vt = string.Empty, _maphieumh = string.Empty, _mancc = string.Empty, _maphieubg = string.Empty, _tenphieumh = string.Empty, _nguoitao = string.Empty, _tienteID = string.Empty, _pomua = string.Empty ,_mtt = string.Empty, _maphieucd = string.Empty;
        bool _IsEdit;
        bool _IsDuyet;
        bool _IsAdd;
        bool _IsView;
        ...
        public frmPhieuMuaHangPCDDH(DataRow rowFocused = null, bool IsEdit = false, bool IsAdd = false, bool IsView = false, bool IsMuaTVT = false)
        {
            InitializeComponent();
            ...
            if (_IsEdit == true || _IsView == true)
            {
                ...
                _maphieucd = _rowFocused["MaPhieuCD"]?.ToString();
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (_IsEdit == true) 
            {
                ...
                //searchLookUpEditPCD.ReadOnly = true;
            ...
            if (_IsView == true) 
            {
                ...
                searchLookUpEditPCD.EditValue = _maphieucd.ToString();
                searchLookUpEditNCC.EditValue = _mancc.ToString();
                //searchLookUpEditPBG.EditValue = _maphieubg.ToString();
                txtNguoiTao.EditValue = _nguoitao;
                DEditNgayTao.EditValue = _NgayTao;
            ...
            //ckDonHang.Checked = true;
            if (!string.IsNullOrEmpty(_maphieucd))
            {
               
                LoadData();
                searchLookUpEditPCD.Properties.ReadOnly = true;
                searchLookUpEditPCD.Enabled = false;
            }
            if (_rowFocused != null)
            {
        ...
        private void LoadData()
        {
            try
            {
                string url = $"{URL}NhaCC/GePMH?action=GETPMHChiTiet&para1={_maphieumh}&para2={ckDonHang.Checked}&para3=&para4=&para5=";
                string json = Task.Run(async () => { return await _clientExtension.GetAsnyc(url); }).Result;

                DataTable apiTable = JsonConvert.DeserializeObject<DataTable>(json);
                if(apiTable != null && apiTable?.Rows?.Count > 0)
                {
                    bool.TryParse(apiTable.Rows[0]["IsDH"]?.ToString(), out bool IsDH);
                    ckDonHang.Checked = IsDH;
                    if (!IsDH)
                    {
                        CreateSearchlookupPCD();
                    }
         
                    searchLookUpEditPCD.EditValue = apiTable.Rows[0]["MaPhieuCD"];
                }
        ...
        {
            string url = $"{URL}NhaCC/GePMH?action=GetKH&para1=&para2=&para3=&para4=&para5=";
            string json = Task.Run(async () => { return await _clientExtension.GetAsnyc(url); }).Result;
            DataTable tbl = JsonConvert.DeserializeObject<DataTable>(json);
            searchLookUpEditNCC.Properties.DataSource = tbl;
            searchLookUpEditNCC.Properties.ValueMember = "MaKH";
            searchLookUpEditNCC.Properties.DisplayMember = "TenKH";

        }

        private void CreateSearchlookupPCD()
        {
            string url = $"{URL}NhaCC/GePMH?action=GetPCDDH&para1={ckDonHang.Checked}&para2=&para3=&para4=&para5=";
            string json = Task.Run(async () => { return await _clientExtension.GetAsnyc(url); }).Result;
            DataTable tbl = JsonConvert.DeserializeObject<DataTable>(json);
            searchLookUpEditPCD.Properties.DataSource = tbl;
            searchLookUpEditPCD.Properties.ValueMember = "MaPhieu";
            searchLookUpEditPCD.Properties.DisplayMember = "TenPhieu";
        ...
        private DataTable CreateTempSelectedTable()
        {
        ...
        public void SaveData(DataTable dt)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                newRow["MaKhachHang"] = searchLookUpEditNCC.EditValue;
        ...
        public void SaveDataDot(DataTable dt)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                newRow["Thue"] = dr["Thue"];
                newRow["PTGiaoHang"] = "";
                newRow["MaPhieuBG"] = dr["MaPhieuBG"] == null ? DBNull.Value : dr["MaPhieuBG"];
                tblDG.Rows.Add(newRow);
            }
        ...
        private void barButtonItem3_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            // 4️⃣ Update summary + tiền
            UpdateGridView5Summary();
            UpdateGridView5Values();

            // 5️⃣ Update số loại vật tư
            int soChungLoai = 0;
            if (dt.Rows.Count > 0)
            {
            ...
            // 6️⃣ Nếu xóa hết → reset maphieubg
            if (dt.Rows.Count == 0)
            {
        ...
        private void btnChonVT_Click(object sender, EventArgs e)
        {
            ...
            string mancc = searchLookUpEditNCC.EditValue.ToString();
            string maphieucd = searchLookUpEditPCD.EditValue.ToString();
            //string maphieubg = searchLookUpEditPBG.EditValue.ToString();
            string maphieubg = string.Empty;
            string matiente = txtTienTe.EditValue.ToString();
            DataTable selectedTable = GetAutoVatTu(mancc,maphieucd);
            if (selectedTable == null || selectedTable.Rows.Count == 0)
                return;
        ...
        private void ckDonHang_CheckedChanged(object sender, EventArgs e)
        {
            ...
            gridControl4.DataSource = new DataTable();
            CreateSearchlookupPCD();
            if (ckDonHang.Checked)
            {
                gridColumn85.Visible = true;
            }
            else
            {
        ...
        private void searchLookUpEditPCD_Properties_EditValueChanged(object sender, EventArgs e)
        {
            _dtTable = new DataTable();
            string maphieu = searchLookUpEditPCD.EditValue == null ? "" : searchLookUpEditPCD.EditValue.ToString();
            string url = $"{URL}NhaCC/GePMH?action=GetKHPCD&para1={maphieu}&para2=&para3=&para4=&para5=";
            string json = Task.Run(async () => { return await _clientExtension.GetAsnyc(url); }).Result;
            DataTable tbl = JsonConvert.DeserializeObject<DataTable>(json);
            if (tbl != null && tbl.Rows.Count > 0)
            {
                //searchLookUpEditPBG.EditValue = tbl.Rows[0]["MaPhieuBG"];
                searchLookUpEditNCC.EditValue = tbl.Rows[0]["MaKH"];
                textEdit1.EditValue = tbl.Rows[0]["TenHang"];
                chonvtphieu();
            }
        }
        private DataTable GetAutoVatTu(string makh,string maphieucd)
        {
            try
            {
                ...
                urlVT = $"{URL}NhaCC/GePMH?action=GetVTPCD&para1={makh}&para2={maphieucd}&para3={ckDonHang.Checked}&para4=&para5=";

                jsonVT = Task.Run(async () => await _clientExtension.GetAsnyc(urlVT)).Result;

                if (string.IsNullOrEmpty(jsonVT) || jsonVT == "[]")
                    return null;

                DataTable tblGrid = JsonConvert.DeserializeObject<DataTable>(jsonVT);

                if (tblGrid == null || tblGrid.Rows.Count == 0)
                    return null;

                DataTable dtSelected = CreateTempSelectedTable(); // giống cấu trúc _tbl bên frm
                ...
                foreach (DataRow row in tblGrid.Rows)
                {
                    ...
                    string urlTyGia = $"{URL}NhaCC/GePMH?action=GETPTyGia&para1={matiente}&para2=VND&para3=&para4=&para5=";
                    ...
                    decimal giaqd = tblTyGia.Rows.Count > 0 ? Convert.ToDecimal(tblTyGia.Rows[0]["TyGia"]) : 0;
                    ...
                    newRow["PTVanChuyen"] = row["PTVanChuyen"];
                    newRow["GhiChu"] = row["GhiChu"];
                    newRow["NgayBDHieuLuc"] = row["NgayBDHieuLuc"];
                    newRow["TienTeID"] = row["TienTeID"];
        ...
        public void Save_E_Invoice()
        {
            try
            {
                ...
                rowEInvoice["MaKhachHang"] = searchLookUpEditNCC.EditValue;
                ...
                DateTime DeclarationDate = DateEditInvoice.EditValue == null ? DateTime.MinValue : (DateTime)DateEditDeclaration.EditValue;
