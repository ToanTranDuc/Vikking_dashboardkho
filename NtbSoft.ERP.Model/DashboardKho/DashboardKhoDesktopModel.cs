...
namespace NtbSoft.ERP.Model.DashboardKho
{
    ...
    public static class DashboardKhoDesktopModel
    {
        ...
        public static DataTable GetVatTuTheoDungTich()
        {
            return ExecuteQuery(@"
                SELECT 
                    ...
                    COUNT(DISTINCT v.Barcode) AS SoRoll,
                    SUM(ISNULL(ct.SoLuongThucTe, 0)) AS SLTon,
                    SUM(ISNULL(ct.SoLuongThucTe, 0) * ISNULL(ct.DonGia, 0)) AS ThanhTien,
                    ROUND(SUM(v.CBM), 4) AS CBM,
                    ROUND(SUM(v.CBM) / NULLIF((SELECT SUM(v2.CBM) FROM dbo.ERP_VatTuCBM v2 WHERE v2.MaONPL IS NOT NULL), 0) * 100, 2) AS TyTrong,
                ...
                LEFT JOIN dbo.ERP_DonViVT d ON d.MaDVVT = ct.MaDVVT
                WHERE v.MaONPL IS NOT NULL
                GROUP BY 
                    vt.MaVT, 
                    vt.ChiTiet, 
            ...
            return ExecuteQuery(@"
                ...
                ;WITH CTE AS (
                    SELECT 
                        ...
                        COUNT(DISTINCT ct.BarCode) AS SoRoll,
                        SUM(ISNULL(cbm_agg.CBM, 0)) AS TongCBM,
                        SUM(ISNULL(ct.SoLuongThucTeBanDau, 0)) AS GiaTri,
                        SUM(ISNULL(ct.SoLuongThucTeBanDau, 0) * ISNULL(CAST(ct.DonGia AS DECIMAL(20,4)), 0)) AS ThanhTien
        ...
        {
            return ExecuteSP("GetTonDauKyChiTiet", cmd => {
                cmd.Parameters.AddWithValue("@TuNgay", tuNgay);
        ...
        {
            return ExecuteSP("GetTonDauKyRollDetail", cmd => {
                cmd.Parameters.AddWithValue("@TuNgay", tuNgay);
            ...
            else if (gb == "dh")
            {
                return ExecuteQuery(@"
                    SELECT 
                        ...
                        SUM(ISNULL(xh.SLNhap, 0) * ISNULL(ct.DonGia, 0)) AS GiaTri,
