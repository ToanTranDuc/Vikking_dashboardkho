    ...
    var html = data
        .map(function (entry) {
            ...
            return (
                ...
                "</td>" +
                "<td>" +
                escapeHtml(item.MaHang || "") +
                "</td>" +
                '<td class="text-end">' +
                formatNumber(item.SoLuongSP, 0) +
                "</td>" +
                "<td>" +
    ...
    var html = state.outboundReady
        ...
        .map(function (item, stt) {
            return (
                ...
                escapeHtml(normalizeCustomerName(item.KhachHang)) +
                "</td>" +
                "<td>" +
                ...
                cols
                    .map(function (c) {
                        ...
                        if (c.number !== undefined)
                            return '<td class="text-end dk-cell-num">' + formatNumber(v, c.number) + "</td>";
...
function renderCapacityBarChart() {
    ...
    function buildGaugeSvg(cx, cy, R, label, pct, used, total, gradId) {
        ...
        html +=
            ...
            '<filter id="' +
            gradId +
            '_glow" x="-30%" y="-30%" width="160%" height="160%">' +
        ...
        if (arc.fill) {
            ...
            html +=
                ...
                strokeW +
                '" stroke-linecap="round" ' +
                'pathLength="1000" stroke-dasharray="1000" filter="url(#' +
                gradId +
                '_glow)"/>';
        }
        ...
        html +=
            ...
            '" y2="' +
            nyOut +
            '" stroke="#ef4444" stroke-width="2.5" stroke-dasharray="3 2"/>';
        html +=
            '<text x="' +
            cx +
            '" y="' +
            (cy - 10) +
            '" text-anchor="middle" class="dk-gauge-pct" style="font-size:32px;font-weight:900;fill:' +
            col +
        ...
        html +=
            ...
            '" y="' +
            (cy + 14) +
            '" text-anchor="middle" style="font-size:12px;font-weight:700;fill:' +
            subTxt +
            ';">' +
            formatNumber(used, 1) +
            " / " +
        ...
        html +=
            ...
            '" y="' +
            (cy + 37) +
            '" text-anchor="middle" style="font-size:13px;font-weight:900;fill:' +
            tagCol +
            ';">' +
            escapeHtml(label) +
            "</text>";
    ...
    function makeSvg(label, pct, used, total, gradId) {
        ...
        inner +=
            ...
            '" y="' +
            (H - 2) +
            '" text-anchor="middle" style="font-size:9px;fill:#ef4444;font-weight:700;">── 85% cảnh báo</text>';
        return '<svg class="dk-gauge-svg" viewBox="0 0 ' + W + " " + H + '">' + inner + "</svg>";
    }

    node.innerHTML =
        '<div class="dk-dual-gauge-wrap">' +
        '<div class="dk-gauge-half">' +
        makeSvg("Kho NL", pctNl, usedNpl, capNpl, "gradNL") +
        "</div>" +
        '<div class="dk-gauge-divider"></div>' +
        '<div class="dk-gauge-half">' +
        makeSvg("Kho PL", pctPl, usedPl, capPl, "gradPL") +
        "</div>" +
        "</div>";
}
...
function showLpcpMonthModal(type) {
    ...
    requestJson(url)
        .then(function (res) {
            ...
            html +=
                '<th style="padding:14px 16px;text-align:left;border-bottom:1px solid var(--dk-line);font-weight:600;color:var(--dk-title);width:45%;">Mã lệnh / Brand</th>';
            html +=
            ...
            for (var k = 0; k < tasks.length; k++) {
                ...
                html +=
                    ...
                    "</b>" +
                    (tk.TenBrand
                        ? ' <span style="color:var(--dk-muted);"> - ' + escapeHtml(tk.TenBrand) + "</span>"
                        : "") +
                    "</td>";
                html +=
        ...
        .then(function (res) {
            ...
            if (thieuNPL.length)
                warnings.push({
                    ...
                    desc: thieuNPL.slice(0, 3).join(", ") + (thieuNPL.length > 3 ? "..." : ""),
                });
            if (thieuPO.length)
                warnings.push({
                    level: "critical",
                    ...
                    cnt: tongThieu + " lo\u1ea1i",
                    desc: thieuPO.slice(0, 3).join(", ") + (thieuPO.length > 3 ? "..." : ""),
                });
            if (chuaHT.length)
                warnings.push({
                    level: "caution",
            ...
            if (assignments.length === 0) {
                ...
                assignments.forEach(function (a) {
                    ...
                    var brand = a.TenBrand
                        ? " \u00b7 " + escapeHtml(a.TenBrand)
                        : a.MaKhachHang
                          ? " \u00b7 " + escapeHtml(a.MaKhachHang)
                          : "";
                    var timeHtml = a.GioThucHien
                        ? "<span style='font-size:10px;color:var(--dk-muted);margin-left:6px;'><i class='far fa-clock'></i> " +
                          escapeHtml(a.GioThucHien) +
                          "</span>"
                        : "";
                    var descHtml = a.MoTaCongViec
                        ? "<div style='font-size:10px;color:var(--dk-muted);margin-top:2px;'><i class='fas fa-tasks' style='margin-right:4px;opacity:0.7'></i>" +
                    ...
                    aHtml +=
                        ...
                        timeHtml +
                        "</div>" +
                        "<div style='font-size:10px;color:var(--dk-title);margin-top:2px;opacity:0.9;'><i class='far fa-user' style='margin-right:4px;opacity:0.7'></i>" +
                        name +
                        "</div>" +
                        descHtml +
                        "</div>" +
            ...
            if (pickOrders.length === 0) {
                ...
                pickOrders.forEach(function (po) {
                    ...
                    var brand = po.TenBrand
                        ? escapeHtml(po.TenBrand)
                        : po.MaKhachHang
                    ...
                    var thieu =
                        (po.SoPLThieu || 0) > 0
                            ? "<span style='font-size:9px;font-weight:700;color:#dc2626;background:rgba(220,38,38,0.15);padding:1px 6px;border-radius:3px;margin-left:4px;'>\u26a0 Thi\u1ebfu " +
                              po.SoPLThieu +
                              "</span>"
                            : "";
                    ...
                    if (po.SoLoaiPL > 0 || po.TongSLCanSoan > 0) {
                        slHtml =
                            "<div style='font-size:10px;color:var(--dk-muted);margin-top:2px;'><i class='fas fa-box-open' style='margin-right:4px;opacity:0.7'></i>Soạn: <b>" +
                            (po.SoLoaiPL || 0) +
                            "</b> loại / <b>" +
                            formatNumber(po.TongSLCanSoan || 0, 2) +
                            "</b> SL</div>";
                    }
                    ...
                    pHtml +=
                        ...
                        (nv
                            ...
                            : "") +
                        slHtml +
                        (thieu ? "<div style='margin-top:4px'>" + thieu + "</div>" : "") +
                        "</div>";
                });
            }
    ...
    if (pickOrders.length === 0) {
        ...
        pickOrders.forEach(function (po) {
            ...
            html +=
                ...
                subColor +
                ';margin-left:5px;">' +
                escapeHtml(po.TenBrand || po.MaKhachHang || "") +
                "</span>" +
                "</div>" +
                '<div style="font-size:10px;color:' +
                subColor +
