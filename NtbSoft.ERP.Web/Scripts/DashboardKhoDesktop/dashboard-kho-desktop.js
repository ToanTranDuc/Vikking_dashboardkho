...
(function () {
    ...
    function renderCapacityChart() {
        ...
        // v2.6.0 — Use pre-calculated per-warehouse percentages from API
        ...
        // Nếu tổng chưa đủ 100 (do làm tròn), điều chỉnh phần trống
        if (totalPct < 99.5) {
    ...
    function applyHighchartsTheme() {
        ...
        Highcharts.setOptions({
            ...
            chart: { style: { fontFamily: "'Segoe UI', Tahoma, Arial, sans-serif" } },
            tooltip: {
    ...
    function renderTop5MinChart() { /* removed in v2.3.18 */ }
    ...
    function renderOverviewDetailTabs(container, nhap, xuat, kiemke, planned) {
        ...
        var tabs = [
            { key: "nhap", label: "Nhập kho", color: "#3b82f6", rows: nhap || [], grouped: true },
            { key: "xuat", label: "Xuất kho", color: "#f97316", rows: addStt(xuat), cols: colsXuat() },
            { key: "kiemke", label: "Kiểm kê", color: "#10b981", rows: addStt(kiemke), cols: colsKiemKe() },
            { key: "plan", label: "NK dự kiến", color: "#8b5cf6", rows: addStt(planned), cols: colsPlanned() }
        ];
        ...
        for (var pi = 0; pi < tabs.length; pi++) {
            ...
            if (tp.rows.length === 0) {
                ...
                panels += renderGroupedNhapTable(tp.rows);
            } else {
                panels += renderDetailTable(tp.cols, tp.rows);
            }
            panels += "</div>";
        }
        ...
        for (var bi = 0; bi < tabBtns.length; bi++) {
            tabBtns[bi].addEventListener("click", function () {
                ...
                if (searchInput && searchInput.value) {
                    filterDetailTable(searchInput.value.trim());
                }
            });
        }
    }

    // v2.3.48 — Render nhập kho grouped by date + PINCC (dùng cho Tổng quát khoảng ngày)
    function renderGroupedNhapTable(rows) {
        if (!rows || rows.length === 0) {
            return "<div class=\"dk-empty\" style=\"padding:20px\">Không có dữ liệu nhập kho</div>";
        }

        // Sort: date asc, PINCC asc, SoLuong desc
        var sorted = rows.slice().sort(function (a, b) {
            var da = String(a.NgayNhap || "").split("T")[0];
            var db = String(b.NgayNhap || "").split("T")[0];
            if (da !== db) return da < db ? -1 : 1;
            var pa = String(a.PINCC || "").toLowerCase();
            var pb = String(b.PINCC || "").toLowerCase();
        ...
        // Group by date
        var groups = [];
        var curDate = null, curGroup = null;
        for (var i = 0; i < sorted.length; i++) {
            var ngay = String(sorted[i].NgayNhap || "").split("T")[0];
            if (ngay !== curDate) {
                curDate = ngay;
                curGroup = { date: ngay, rows: [] };
                groups.push(curGroup);
            }
            curGroup.rows.push(sorted[i]);
        }

        var cols = [
            { key: "STT", label: "STT", center: true, width: 45 },
            { key: "PINCC", label: "PI NCC", center: true },
            { key: "PO", label: "PO", center: true },
            { key: "ItemCode", label: "Itemcode", center: true },
            { key: "MauVT", label: "Màu", center: true },
            { key: "WidthSize", label: "Width/Size", center: true },
            { key: "TenKH", label: "Khách hàng", left: true },
            { key: "SoLuong", label: "Số lượng", number: 2 },
        ...
        var html = "<table class=\"dk-detail-table\"><thead><tr>";
        for (var ci = 0; ci < cols.length; ci++) {
            var c = cols[ci];
            var st = c.center ? ";text-align:center" : (c.number !== undefined ? ";text-align:right" : "");
            if (c.width) st += ";width:" + c.width + "px";
            html += "<th style=\"padding:11px 12px" + st + "\">" + escapeHtml(c.label) + "</th>";
        }
        html += "</tr></thead><tbody>";

        for (var gi = 0; gi < groups.length; gi++) {
            var g = groups[gi];
            var dateLabel = fmtDate(g.date);

            // Date group header row
            html += "<tr class=\"dk-group-date-row\"><td colspan=\"" + nCols + "\">" +
                "<span class=\"dk-group-date-badge\">📅 Ngày " + escapeHtml(dateLabel) + "</span></td></tr>";

            var dayTotal = 0;
            for (var ri = 0; ri < g.rows.length; ri++) {
                var row = g.rows[ri];
                dayTotal += toNumber(row.SoLuong);
                html += "<tr>";
                for (var cj = 0; cj < cols.length; cj++) {
                    ...
                    } else if (col.number !== undefined) {
                        var align = col.center ? "center" : "right";
                        html += "<td style=\"text-align:" + align + "\">" + formatNumber(val, col.number) + "</td>";
                    } else if (col.left) {
                        html += "<td style=\"text-align:left\">" + escapeHtml(val || "") + "</td>";
                    } else if (col.center) {
                        html += "<td style=\"text-align:center\">" + escapeHtml(val || "") + "</td>";
                    } else {
            ...
            // Daily subtotal row: merge first (nCols-2) cols as label, then value, then empty
            html += "<tr class=\"dk-group-total-row\">" +
                "<td colspan=\"" + (nCols - 2) + "\" class=\"dk-total-label\">Tổng ngày " + escapeHtml(dateLabel) + "</td>" +
                "<td class=\"dk-total-value\">" + formatNumber(dayTotal, 2) + "</td>" +
                "<td></td>" +
                "</tr>";
        }
    ...
    function renderDayDetailTabs(container, nhap, xuat, kiemke, planned) {
        ...
        // v2.3.45 — Inject STT vào mọi row cho tất cả 4 tabs
        function addStt(arr) {
            return (arr || []).map(function (r, i) { return Object.assign({ STT: i + 1 }, r); });
        }
        var tabs = [
            { key: "nhap", label: "Nhập kho", color: "#3b82f6", rows: addStt(nhap), cols: colsNhap() },
            { key: "xuat", label: "Xuất kho", color: "#f97316", rows: addStt(xuat), cols: colsXuat() },
            { key: "kiemke", label: "Kiểm kê", color: "#10b981", rows: addStt(kiemke), cols: colsKiemKe() },
            { key: "plan", label: "NK dự kiến", color: "#8b5cf6", rows: addStt(planned), cols: colsPlanned() }
        ];

        // Pick default active tab — first non-empty, else "nhap"
        var activeIdx = 0;
        for (var ti = 0; ti < tabs.length; ti++) {
            if (tabs[ti].rows.length > 0) { activeIdx = ti; break; }
        }
        ...
        for (var bi = 0; bi < tabBtns.length; bi++) {
            tabBtns[bi].addEventListener("click", function () {
                ...
                // v2.3.9 — Re-apply search filter on new tab
                var searchInput = byId("detailSearchInput");
                if (searchInput && searchInput.value) {
                    filterDetailTable(searchInput.value.trim());
                }
        ...
        if (document.getElementById("chartVolumePie")) {
            Highcharts.chart("chartVolumePie", {
                        ...
                        render: function () {
                            ...
                            if (series && series.center) {
                                ...
                                chart.centerLabel = chart.renderer.text(
                                    '<div style="text-align:center;font-size:14px;line-height:1.2;color:' + textColor + ';">Tổng<br><span style="font-size:20px;font-weight:700;">' + Highcharts.numberFormat(sumTotal, 0, '.', '.') + '</span></div>',
                                    cx, cy, true
                ...
                legend: {
                    ...
                    itemStyle: { color: textColor, fontWeight: 'normal', fontSize: '14px' },
                    useHTML: true,
                    labelFormatter: function () {
                        var pct = sumTotal > 0 ? (this.y / sumTotal * 100).toFixed(1) : 0;
                        return '<span style="color:' + textColor + ';">' + this.name + ': <b>' + pct + '%</b></span>';
                    }
                },
        ...
        if (document.getElementById("chartTrendLine")) {
            ...
            doFetch(urlTrend).then(function (data) {
                ...
                Highcharts.chart("chartTrendLine", {
                    chart: { type: "spline", backgroundColor: bgColor, marginTop: 30, marginBottom: 45 },
                    title: { text: '' },
                    ...
                    legend: { layout: 'horizontal', align: 'center', verticalAlign: 'bottom', y: 15, itemStyle: { color: textColor, fontWeight: 'normal', fontSize: '14px' } },
        ...
        if (document.getElementById("chartLoadBar")) {
            ...
            doFetch(urlLoad).then(function (data) {
                ...
                Highcharts.chart("chartLoadBar", {
                    chart: { type: "column", backgroundColor: bgColor, marginTop: 30, marginBottom: 45 },
                    title: { text: '' },
                    xAxis: { categories: cats, labels: { style: { color: textColor, fontSize: '14px' } }, tickLength: 0, lineColor: isDark ? '#334155' : '#e2e8f0' },
                    yAxis: {
            ...
            .then(function (res) {
                ...
                if (assignments.length === 0) {
                    ...
                    assignments.forEach(function (a) {
                        ...
                        var brand = a.TenBrand ? " \u00b7 " + escapeHtml(a.TenBrand) : "";
                        aHtml += "<div style='display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid var(--dk-line);'>" +
                            "<div style='flex:1;min-width:0;'>" +
                            "<div style='font-size:11px;font-weight:600;color:var(--dk-primary,#2563eb);'>" + maLenh + brand + "</div>" +
                            "<div style='font-size:10px;color:var(--dk-title);margin-top:1px;opacity:0.9;'>" + name + "</div>" +
                ...
                if (pickOrders.length === 0) {
                    ...
                    pickOrders.forEach(function (po) {
                        ...
                        pHtml += "<div style='padding:7px 0;border-bottom:1px solid var(--dk-line);'>" +
                            ...
                            "<span style='font-size:11px;font-weight:600;color:var(--dk-primary,#2563eb);'>" + maLenh + "</span>" +
                            ttBadge(po.TrangThai || 0) + "</div>" +
                            (brand ? "<div style='font-size:10px;color:var(--dk-title);margin-top:1px;opacity:0.9;'>" + brand + "</div>" : "") +
                            (nv ? "<div style='font-size:10px;color:var(--dk-title);opacity:0.9;'>" + nv + "</div>" : "") +
                            thieu + "</div>";
                    });
                }
    ...
    function colsNhap() {
        // v2.3.45 — STT đầu tiên; còn: PI NCC, PO, Itemcode, Màu, Width/Size, KH, SL, SoBarCode
        return [
    ...
    function colsXuat() {
        // v2.3.45 — GROUP BY MaLenh; thêm TenHang; bỏ MaNPL/BarCode level
        return [
            { key: "STT", label: "STT", number: 0, center: true },
            { key: "MaLenh", label: "Mã lệnh", center: true },
            { key: "TenHang", label: "Tên hàng" },
            { key: "TenKH", label: "Khách hàng" },
            { key: "SoLuong", label: "Số lượng", number: 2, center: true },
            { key: "SoBarCode", label: "Số BarCode", number: 0, center: true }
        ];
    }
    function colsKiemKe() {
        // v2.3.45 — GROUP BY PhieuKiemKe+SoLo; bỏ BarCode/IsXacNhan; thêm SoBarCode
        return [
            { key: "STT", label: "STT", number: 0, center: true },
            { key: "PhieuKiemKe", label: "Phiếu KK", center: true },
            { key: "SoLo", label: "Số lô", center: true },
            { key: "SoBarCode", label: "Số BarCode", number: 0, center: true },
            { key: "SoLuong", label: "SL kiểm kê", number: 2, center: true },
            { key: "UserKK", label: "Người KK", center: true },
            { key: "GhiChu", label: "Ghi chú" }
        ];
    }
    function colsPlanned() {
        // v2.3.45 — Thêm STT đầu tiên
        return [
            { key: "STT", label: "STT", number: 0, center: true },
            { key: "SoLo", label: "Số lô", center: true },
            { key: "PO", label: "PO", center: true },
            { key: "MaKH", label: "Mã KH", center: true },
            { key: "TenKH", label: "Khách hàng" },
            { key: "SoLuongDuKien", label: "Số lượng dự kiến", number: 0 },
            { key: "NgayNKDuKien", label: "Ngày dự kiến", date: true, center: true }
        ];
    }
    ...
    function getDetailData(detail, index) {
        ...
        var detailMap = {
            ...
            inboundAll: {
                ...
                columns: [
                    { key: "STT", label: "STT", number: 0, center: true, width: 50 },
                    { key: "PO", label: "PO", center: true, width: 140 },
                    { key: "SoLo", label: "Số lô", center: true, width: 120 },
                    { key: "MaDH", label: "Mã đơn hàng", center: true, width: 120 },
                    { key: "MaHang", label: "Mã hàng", center: true, width: 150 },
                    { key: "TenHang", label: "Tên hàng", width: 220 },
    ...
    function renderDetailTable(columns, rows) {
        ...
        var headHtml = columns.map(function (column) {
            ...
            if (column.width) styles.push("width:" + column.width + "px");
    ...
    function renderDetailModal(detail, index) {
        var model;
        try { model = getDetailData(detail, index); } catch (e) { model = { title: detail, rows: [], columns: [], isWarehouseMap: false }; }
    ...
    function bindThemeToggle() {
        ...
        function doThemeToggle() {
            ...
            try { localStorage.setItem("dkTheme", isDark ? "dark" : "light"); } catch (e) { }
            try { applyHighchartsTheme(); } catch (e) { }
            try { renderCustomerPieChart(); } catch (e) { }
            try { renderCapacityChart(); } catch (e) { }
            try { renderTop5MaxNLChart(); } catch (e) { }
            try { renderTop5MaxPLChart(); } catch (e) { }
            try { renderFlowTrendChart(); } catch (e) { }
            try { renderAgeStockChart(); } catch (e) { }
            try { renderGiaTriTheoNhomChart(); } catch (e) { }
            try { renderHieuSuatGauges(); } catch (e) { }
        }
    ...
    function renderTop5KHAllModal() {
        ...
        requestJson("/api/DashboardKhoDesktop/GetKhachHangTonKhoChiTiet").then(function (data) {
            ...
            var tableHtml = '<table class="dk-detail-table dk-top5kh-table"><thead><tr>' +
            ...
            for (var i = 0; i < groups.length; i++) {
                ...
                if (subList.length > 0) {
                    ...
                    for (var j = 0; j < subList.length; j++) {
                        var s = subList[j];
                        html += '<tr><td class="dk-cell-mono">' + escapeHtml(s.MaVT || "") + '</td>' +
                            '<td>' + escapeHtml(s.Nhom || "") + '</td>' +
                            '<td class="text-end dk-cell-num">' + formatNumber(toNumber(s.TongCBM), 4) + '</td>' +
                            '<td class="text-end dk-cell-num">' + formatNumber(toNumber(s.GiaTri), 0) + '</td></tr>';
                    }
    ...
    function renderTonDauKyDetailModal() {
        ...
        function paint() {
            ...
            content.innerHTML =
                ...
                '<div id="kpiTbody">' + modalLoading() + '</div>';
            bindKpiSubtabs();
        ...
        function doFilter() {
            ...
            var filtered = allRows.filter(function (r) {
                if (!q) return true;
                var hay = ((r.MaNPL || "") + " " + (r.ItemCode || "") + " " + (r.TenVT || "") + " " + (r.Mau || "") + " " + (r.KhoVai || "")).toLowerCase();
    ...
    function renderSortableTable(cols, rows, opts) {
        ...
        for (var i = 0; i < cols.length; i++) {
            ...
            head += '<th' + thCls + thStyleStr + '>' + escapeHtml(c.label) +
        ...
        return rows.map(function (r, idx) {
            ...
            return '<tr' + trCls + '>' + cols.map(function (c) {
                ...
                if (c.drillTo && c.key === "MaVT") {
                    return '<td class="dk-cell-mono"><a href="#" class="dk-link-inline" data-mavt-drill="1">' + escapeHtml(v || "") + '</a></td>';
                }
                if (c.number !== undefined) return '<td class="text-end dk-cell-num">' + formatNumber(v, c.number) + '</td>';
                if (c.percent) return '<td class="text-end dk-cell-num">' + formatNumber(v, 2) + '%</td>';
    ...
    function initFlatpickr() {
        if (typeof flatpickr !== 'undefined') {
            flatpickr("#dashFromDate, #dashToDate, #flowFromDate, #flowToDate, #calFromDate, #calToDate", {
                dateFormat: "Y-m-d",
                locale: "vn"
            });
        }
