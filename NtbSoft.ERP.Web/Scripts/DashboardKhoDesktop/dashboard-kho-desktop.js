...
(function () {
    ...
    function __processRequestQueue() {
        ...
        __activeRequests++;
        __requestJsonCore(req.url).then(function (res) {
            req.resolve(res);
        }).catch(function (err) {
            req.reject(err);
        }).finally(function () {
    ...
    function openDetail(detail, index) {
        ...
        var modal = byId(ids.detailModal);
        if (modal && content) {
            content.innerHTML = '<div style="padding:40px;text-align:center"><div class="dk-spinner" style="margin:0 auto 10px auto;border-top-color:#3b82f6;"></div><div style="color:#6b7280;font-size:13px">Đang tải dữ liệu...</div></div>';
            modal.classList.add("open");
            setTimeout(function () {
