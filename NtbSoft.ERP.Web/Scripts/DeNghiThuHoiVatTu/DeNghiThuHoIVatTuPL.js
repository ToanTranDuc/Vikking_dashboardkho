...
function renderSignCell(container, options, fieldName) {
    ...
    if (options.data[fieldName] && options.data[fieldName] !== "checked") {
        ...
        const $wrapper = $("<div>").css({ display: "flex", flexDirection: "column", alignItems: "center", gap: "2px" });
        $("<img>").attr("src", `/Images/SignDeNghiThuHoi/${options.data[fieldName]}?${Date.now()}`)
        ...
        if (hinhAnh) {
            ...
            $("<span>").text(userName || tenOnly).css({ fontSize: "12px", color: "#333", fontWeight: "bold" }).appendTo(containerDiv);
        }
    }
    ...
    $("<input>").attr("type", "checkbox").addClass(checkSign)
        .css({ cursor: "pointer", left: "4px", top: "5px", position: "absolute", width: "14px", height: "14px", accentColor: "#007bff" })
        .on("change", async function () {
            const rowData = options.data;
            let arrSave = [{
    ...
    $("<i>").addClass(`fa-solid fa-signature ${checkSign}`)
        .css({ cursor: "pointer", color: "#007bff", fontSize: "14px", position: "absolute", top: "2px", right: "4px" })
        .on("click", function () {
            const rowData = options.data;
            showConfirmModalSign(async function () {
                const signatureImage = canvas.toDataURL('image/png');
